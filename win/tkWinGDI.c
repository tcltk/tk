 ...
 * Helper functions.
 */
static int GdiMakeLogFont(Tcl_Interp *interp, const char *str, LOGFONTW *lf, HDC hDC);
...
static int GdiMakeLogFont(Tcl_Interp *interp, const char *str, LOGFONTW *lf, HDC hDC)
{
	...
	Tcl_DString ds;
	Tcl_DStringInit(&ds);
	wcsncpy(lf->lfFaceName, Tcl_UtfToWCharDString(list[0], -1, &ds), sizeof(lf->lfFaceName) - 1);
	Tcl_DStringFree(&ds);
    } else {
	return 0;
    }
    ...
    Tcl_Obj *const objv[])
{
    ...
    Tcl_DStringInit(&ds);
    if ((OpenPrinterW(Tcl_UtfToWCharDString(printer, -1, &ds), (LPHANDLE)&printDC, NULL)) == FALSE) {
