...
static int		TkGdiMakeBezierCurve(Tk_Canvas, double *, int, int,
			    XPoint[], double[]);
 ...
 *
 *	Note: in spite of this procedure's name, it does *not* generate Bezier
 *	curves.  Since only three control points are used for each curve
 *	segment, not four, the curves are actually just parabolic.
 *
 * Results:
 *
 ...
 *
 *	Note: if the first and last points are the same, then a closed curve
 *	is generated.
 *
 * Side effects:
 *	None.
...
TkGdiMakeBezierCurve(
    Tk_Canvas canvas,		/* Canvas in which curve is to be drawn.*/
    double *pointPtr,		/* Array of input coordinates:
				 *    x0, y0, x1, y1, etc... */
