				 ...
				 * application. */
{
    ...
    sprintf(id, "%lx ", (unsigned long) commWindow);
    idLength = strlen(id);
    ...
    Tcl_Obj *const objv[])	/* Argument strings. */
{
	...
	sprintf(buffer, "%lx %d",
		(unsigned long) Tk_WindowId(dispPtr->commTkwin),
		localData.sendSerial);
     ...
     * needed by SendEventProc to pass back the
				 ...
				 * lookup. */
{
	...
	if (sscanf(p, "%lx", (unsigned long *) &id) != 1) {
	    commWindow = None;
	} else {
	    commWindow = id;
	}
