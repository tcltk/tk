...
//------------------------------------------------------------------------
void Agg2D::miterLimit(double g)
{
    m_convStroke.miter_limit(g);
}
...
{
   return m_fontEngine.ascender();
}


//------------------------------------------------------------------------
double Agg2D::fontDescent() const
{
   return m_fontEngine.descender();
}
...
//------------------------------------------------------------------------
double Agg2D::textWidthU(const unsigned* str, int len)
{
    double x = 0;
    double y = 0;
    bool first = true;
...
//------------------------------------------------------------------------
void Agg2D::textU(double x, double y, const unsigned* str, int len, bool roundOff, double ddx, double ddy)
{
    double dx = 0.0;
    double dy = 0.0;
    ...
    switch(m_textAlignX)
    {
        case AlignCenter:  dx = -textWidthU(str, len) * 0.5; break;
        case AlignRight:   dx = -textWidthU(str, len);       break;
        default: break;
    }
...
//------------------------------------------------------------------------
void Agg2D::imageWrapMode(WrapMode f)
{
    m_imageWrapMode = f;
}
...
//------------------------------------------------------------------------
Agg2D::WrapMode Agg2D::imageWrapMode() const
{
    return m_imageWrapMode;
}
                            ...
                            BaseRenderer& renBase, Interpolator& interpolator)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (resample)
                {
                    typedef agg::span_image_resample_rgba_affine<img_source_type_wrap> SpanGenType;
                    typedef agg::span_converter<SpanGenType,SpanConvImageBlend> SpanConvType;
                    typedef agg::renderer_scanline_aa<BaseRenderer,Agg2D::SpanAllocator,SpanGenType> RendererType;
                ...
                else
                {
                    ...
                    else
                    {
                        if (gr.m_imageFilterLut.diameter() == 2)
                        {
                            typedef agg::span_image_filter_rgba_2x2<img_source_type_wrap,Interpolator> SpanGenType;
                            typedef agg::span_converter<SpanGenType,SpanConvImageBlend> SpanConvType;
                            typedef agg::renderer_scanline_aa<BaseRenderer,Agg2D::SpanAllocator,SpanGenType> RendererType;
