...
namespace agg
{
    ...
    double font_engine_freetype_base::ascender() const
    {
        if(m_cur_face)
        {
            return (double) m_cur_face->size->metrics.ascender / 64.0;
        }
        return 0.0;
    }
    ...
    double font_engine_freetype_base::descender() const
    {
        if(m_cur_face)
        {
            return (double) m_cur_face->size->metrics.descender / 64.0;
        }
        return 0.0;
    }
                                              ...
                                              const long font_mem_size)
    {
        ...
        if(m_library_initialized)
        {
            ...
            if(idx >= 0)
            {
                ...
                {
                    FT_Stream strm = (FT_Stream)font_mem;
                    strm->close(strm);
                }
            }
            else
            {
                ...
                else if (font_mem)
                {
                    ...
                    args.stream = (FT_Stream)font_mem;
                    m_last_error = FT_Open_Face(m_library,
                                                &args,
                                                face_index,
                                                &m_faces[m_num_faces]);
                }
    ...
    bool font_engine_freetype_base::prepare_glyph(unsigned glyph_code)
    {
        ...
        m_last_error = FT_Load_Glyph(m_cur_face, 
                                     m_glyph_index, 
                                     (m_hinting ? FT_LOAD_DEFAULT : FT_LOAD_NO_HINTING) | FT_LOAD_NO_BITMAP);
