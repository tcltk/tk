...
namespace agg
{
    ...
    struct blender_bgr565
    {
                                         ...
                                         unsigned)
        {
            ...
            calc_type g = (rgb >> 3) & 0xFC;
            calc_type b = (rgb >> 8) & 0xF8;
            *p = (pixel_type)
               ( (((cr - r) * alpha + (r << 8)) >> 11)          |
                ((((cg - g) * alpha + (g << 8)) >> 5) & 0x07E0) |
		((((cb - b) * alpha + (b << 8))     ) & 0xF800));
        }

        static AGG_INLINE pixel_type make_pix(unsigned r, unsigned g, unsigned b)
        {
	    return (pixel_type)((r >> 3) | ((g & 0xFC) << 3) | ((b & 0xF8) << 8));
        }
    ...
    //=====================================================blender_bgr565_gamma
    template<class Gamma> class blender_bgr565_gamma
    {
    public:
        typedef rgba8 color_type;
        typedef color_type::value_type value_type;
        ...
        blender_bgr565_gamma() : m_gamma(0) {}
        void gamma(const gamma_type& g) { m_gamma = &g; }
        ...
        {
            pixel_type rgb = *p;
            calc_type r = m_gamma->dir((rgb << 3) & 0xF8);
            calc_type g = m_gamma->dir((rgb >> 3) & 0xFC);
            calc_type b = m_gamma->dir((rgb >> 8) & 0xF8);
            *p = (pixel_type)
               ( (m_gamma->inv(((m_gamma->dir(cb) - r) * alpha + (r << 8)) >> 8) >> 3)           |
                ((m_gamma->inv(((m_gamma->dir(cg) - g) * alpha + (g << 8)) >> 8) << 3) & 0x07E0) |
		((m_gamma->inv(((m_gamma->dir(cb) - b) * alpha + (b << 8)) >> 8) << 8) & 0xF800));
        }
    ...
    typedef pixfmt_alpha_blend_rgb_packed<blender_bgr565, rendering_buffer> pixfmt_bgr565; //----pixfmt_bgr565
    ...
    typedef pixfmt_alpha_blend_rgb_packed<blender_bgr565_pre, rendering_buffer> pixfmt_bgr565_pre; //----pixfmt_bgr565_pre
    ...
    //-----------------------------------------------------pixfmt_bgr565_gamma
    template<class Gamma> class pixfmt_bgr565_gamma : 
    public pixfmt_alpha_blend_rgb_packed<blender_bgr565_gamma<Gamma>, rendering_buffer>
    {
    public:
        pixfmt_bgr565_gamma(rendering_buffer& rb, const Gamma& g) :
            pixfmt_alpha_blend_rgb_packed<blender_bgr565_gamma<Gamma>, rendering_buffer>(rb) 
        {
            this->blender().gamma(g);
        }
    };
