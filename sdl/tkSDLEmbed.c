				 ...
				 * tkwin; must be an integer value. */
{
    ...
    idObj = Tcl_NewStringObj(string, -1);
    if (Tcl_GetLongFromObj(interp, idObj, (long *) &id) != TCL_OK) {
				 ...
				 * the window is to be created. */
{
    ...
    w = XCreateWindow(winPtr->display, parent, winPtr->changes.x,
    ...
    TkWindow *winPtr)		/* Tk's structure for an embedded window. */
{
    ...
    Tcl_Panic("TkSDLContainerId couldn't find window");
    return None;
}

/*
...
#define PROP_EVENTS \
    ...
    SdlTkAttachTkWindow(winPtr->window, winPtr);
}

/*
 *----------------------------------------------------------------------
