				 ...
				 * mapped. */
{
	...
	XRaiseWindow(winPtr->display, wmPtr->wrapperPtr->window);
    }
}

/*
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
	...
	if (wmPtr2->wrapperPtr == NULL) {
	    ...
	    ((TkWindow *) tkwin2)->flags |= TK_APP_TOP_LEVEL;
	}
	if (wmPtr->leaderName != NULL) {
	    ckfree(wmPtr->leaderName);
	}
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
	...
	if (wmPtr2->wrapperPtr == NULL) {
	    CreateWrapper(wmPtr2);
	    masterPtr->flags |= TK_APP_TOP_LEVEL;
	}
    ...
    XEvent *eventPtr)		/* Event that just happened. */
{
	...
	if (wmPtr->menubar != NULL) {
	    TkWindow *mbarPtr = (TkWindow *) wmPtr->menubar;

	    if (mbarPtr->window != None) {
		XMapWindow(wmPtr->winPtr->display, mbarPtr->window);
	    }
	}
	goto doMapEvent;
    } else if (eventPtr->type == UnmapNotify) {
	...
	if (wmPtr->menubar != NULL) {
	    ...
	    if (mbarPtr->window != None) {
		XUnmapWindow(wmPtr->winPtr->display, mbarPtr->window);
	    }
	}
	goto doMapEvent;
    ...
    TkWindow *winPtr)
{
    ...
    if (strcmp(".", winPtr->pathName) == 0) {
	SDL_SetWindowTitle(SdlTkX.sdlscreen, string);
    }
...
TkSetTransientFor(Tk_Window tkwin, Tk_Window parent)
{
	...
	((TkWindow *)parent)->flags |= TK_APP_TOP_LEVEL;
    }
				 ...
				 * window. */
{
    ...
    SdlTkAttachTkWindow(wrapperPtr->window, wrapperPtr);
				 ...
				 * 0. */
{
	...
	if (wmPtr->wrapperPtr == NULL) {
	    CreateWrapper(wmPtr);
	    ((TkWindow *) tkwin)->flags |= TK_APP_TOP_LEVEL;
	}
