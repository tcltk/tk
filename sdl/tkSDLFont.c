...
typedef struct UnixFont {
    ...
    unsigned char widths[BASE_CHARS];	/* Widths of first BASE_CHARS chars
				 	 * in the base font,
				 ...
				 * terminating character. */
{
	...
	familyPtr = lastSubFontPtr->familyPtr;
	for (p = source; ; ) {
	    if ((ch < BASE_CHARS) && (fontPtr->widths[ch] != 255)) {
 ...
 * DrawChars --
 *
 *	Draw a string of characters on the screen. DrawChars() expands
...
Tk_DrawChars(
    ...
    int x, int y)		/* Coordinates at which to place origin of
				 * string when drawing. */
{
    DrawChars(display, drawable, gc, tkfont, source, numBytes, x, y);
}

/*
 *---------------------------------------------------------------------------
...
{
    (void) numBytes; /*unused*/
    DrawChars(display, drawable, gc, tkfont, source + rangeStart,
	      rangeLength, x, y);
}

/*
...
CanUseFallback(
    ...
    SubFont **fixSubFontPtrPtr)	/* Subfont reference to fix up if we
				 * reallocate our subfont table. */
{
    int i, nameIdx, numNames, numEncodings, bestIdx[2];
    int retries;
    unsigned bestScore[2];
    char **nameList;
    char **nameListOrig;
...
{
    return SdlTkListFonts(faceName, numNamesPtr);
}
				 ...
				 * if no names were found. */
{
    ...
    if (aliases != NULL) {
	for (i = 0; aliases[i] != NULL; i++) {
	    if (strcasecmp(faceName, aliases[i]) == 0) {
		continue;
	    }
    ...
    double angle)
{
	...
	if (fontPtr->font.fa.underline != 0) {
	    dy = fontPtr->underlinePos;
	    points[0].x = xStart + ROUND16(dy*sinA);
	    points[0].y = yStart + ROUND16(dy*cosA);
	    points[1].x = x + ROUND16(dy*sinA);
	    points[1].y = y + ROUND16(dy*cosA);
	    points[2].x = x + ROUND16(dy*sinA + fontPtr->barHeight*sinA);
            points[2].y = y + ROUND16(dy*cosA + fontPtr->barHeight*cosA);
            points[3].x = xStart + ROUND16(dy*sinA + fontPtr->barHeight*sinA);
            points[3].y = yStart + ROUND16(dy*cosA + fontPtr->barHeight*cosA);
	...
	if (fontPtr->font.fa.overstrike != 0) {
	    dy = fontPtr->font.fm.descent + (fontPtr->font.fm.ascent) / 10;
	    dy = -dy;
	    points[0].x = xStart + ROUND16(dy*sinA);
	    points[0].y = yStart + ROUND16(dy*cosA);
	    points[1].x = x + ROUND16(dy*sinA);
