...
SdlTkTranslateEvent(SDL_Event *sdl_event, XEvent *event, unsigned long now_ms)
{
	...
	break;
    case SDL_FINGERDOWN:
	EVLOG("EV=FINGERDOWN   X=%g Y=%g ID=%" TCL_LL_MODIFIER "d dx=%g dy=%g",
	      sdl_event->tfinger.x, sdl_event->tfinger.y,
	      (Tcl_WideInt) sdl_event->tfinger.fingerId,
	      sdl_event->tfinger.dx, sdl_event->tfinger.dy);
	break;
    case SDL_FINGERUP:
	EVLOG("EV=FINGERUP     X=%g Y=%g ID=%" TCL_LL_MODIFIER "d dx=%g dy=%g",
	      sdl_event->tfinger.x, sdl_event->tfinger.y,
	      (Tcl_WideInt) sdl_event->tfinger.fingerId,
	      sdl_event->tfinger.dx, sdl_event->tfinger.dy);
	break;
    case SDL_FINGERMOTION:
	EVLOG("EV=FINGERMOTION X=%g Y=%g ID=%" TCL_LL_MODIFIER "d dx=%g dy=%g",
	      sdl_event->tfinger.x, sdl_event->tfinger.y,
	      (Tcl_WideInt) sdl_event->tfinger.fingerId,
	      sdl_event->tfinger.dx, sdl_event->tfinger.dy);
	break;
...
doNormalKeyEvent:
	...
	if (translate_zoom) {
		...
		if ((SdlTkX.arg_nogl > 0) && (SdlTkX.root_w == 0)) {
		    /* integral scaling */
		    factor = 0.5;
		}
	...
	sdlwin = SDL_GetWindowFromID(sdl_event->window.windowID);
	if (sdlwin != SdlTkX.sdlscreen) {
	    /*
	     * Maybe another window used for OpenGL rendering,
	     * ignore event.
	     */
	    return 0;
	}
...
SdlTkSetCursor(TkpCursor cursor)
{
    ...
    if (hPtr == NULL) {
	if (SdlTkX.is_framebuffer) {
	    const unsigned char *data = NULL, *mask = NULL;
	    int x, y;

	    /*
	     * Temporary hack for RPI and KMSDRM video drivers.
	     */
		...
		static const unsigned char d[] = {
		    0x00, 0x00, 0x77, 0x00, 0x1c, 0x00, 0x08, 0x00,
		    0x08, 0x00, 0x08, 0x00, 0x08, 0x00, 0x08, 0x00,
		    0x08, 0x00, 0x08, 0x00, 0x08, 0x00, 0x08, 0x00,
		    0x08, 0x00, 0x1c, 0x00, 0x77, 0x00, 0x00, 0x00
		};
		static const unsigned char m[] = {
		    0xf7, 0x80, 0xff, 0x80, 0xff, 0x80, 0x3e, 0x00,
		...
		static const unsigned char d[] = {
		    0x1f, 0xe0, 0x1f, 0xe0, 0x1f, 0xe0, 0x3f, 0xf0,
		    0x61, 0x18, 0xc1, 0x0c, 0x81, 0x07, 0x83, 0x87,
		    0x83, 0x87, 0x84, 0x07, 0xc8, 0x0c, 0x60, 0x18,
		    0x3f, 0xf0, 0x1f, 0xe0, 0x1f, 0xe0, 0x1f, 0xe0
		};
		static const unsigned char m[] = {
		    0x3f, 0xf0, 0x3f, 0xf0, 0x3f, 0xf0, 0x7f, 0xf8,
		...
		static const unsigned char d[] = {
		    0x00, 0x18, 0x00, 0x78, 0x01, 0xe0, 0x03, 0xc0,
		    0x07, 0x80, 0x0f, 0xc0, 0x1f, 0xe0, 0x5f, 0xc0,
		    0xff, 0xe0, 0xbf, 0xe0, 0x0f, 0xc0, 0x0f, 0x80,
		    0x94, 0x00, 0xc4, 0x00, 0x68, 0x00, 0x30, 0x00
		};
		static const unsigned char m[] = {
		    0x00, 0x38, 0x00, 0xf8, 0x03, 0xf0, 0x07, 0xe0,
		    0x0f, 0xc0, 0x1f, 0xe0, 0x7f, 0xf0, 0xff, 0xf0,
		...
		static const unsigned char d[] = {
		    0x00, 0x00, 0x10, 0x10, 0x30, 0x18, 0x7f, 0xfc,
		    0xf0, 0x1e, 0x7f, 0xfc, 0x30, 0x18, 0x10, 0x10,
		    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
		};
		static const unsigned char m[] = {
		    0x18, 0x30, 0x38, 0x38, 0x7f, 0xfc, 0xff, 0xfe,
		    0xff, 0xfe, 0xff, 0xfe, 0x7f, 0xfc, 0x38, 0x38,
		...
		static const unsigned char d[] = {
		    ...
		    0x14, 0x00, 0x14, 0x00, 0x14, 0x00, 0x7f, 0x00,
		    0x3e, 0x00, 0x1c, 0x00, 0x08, 0x00, 0x00, 0x00
		};
		static const unsigned char m[] = {
		    0x1c, 0x00, 0x3e, 0x00, 0x7f, 0x00, 0xff, 0x80,
		    0xff, 0x80, 0x3e, 0x00, 0x3e, 0x00, 0x3e, 0x00,
		    0x3e, 0x00, 0x3e, 0x00, 0xff, 0x80, 0xff, 0x80,
		    0x7f, 0x00, 0x3e, 0x00, 0x1c, 0x00, 0x00, 0x00
		};
		...
		static const unsigned char d[] = {
		    ...
		    0x03, 0x00, 0x23, 0x10, 0x63, 0x18, 0xff, 0xfc,
		    0xff, 0xfc, 0x63, 0x18, 0x23, 0x10, 0x03, 0x00,
		    0x0f, 0xc0, 0x07, 0x80, 0x03, 0x00, 0x00, 0x00
		};
		static const unsigned char m[] = {
		    0x03, 0xc0, 0x03, 0xe0, 0x07, 0xe0, 0x0f, 0xf0,
		    0x17, 0xe8, 0x3b, 0xdc, 0xff, 0xff, 0xff, 0xff,
		    0xff, 0xff, 0xff, 0xff, 0x3b, 0xdc, 0x17, 0xe8,
		...
		static const unsigned char d[] = {
		    0x07, 0x80, 0x0f, 0xc0, 0x1f, 0xe0, 0x33, 0x30,
		    0x33, 0x30, 0x1f, 0xe0, 0x0f, 0xc0, 0x07, 0x80,
		    0x87, 0x84, 0x87, 0x86, 0x43, 0x08, 0x38, 0x70,
		    0x07, 0x80, 0x1f, 0xe2, 0xf0, 0x3e, 0x80, 0x04
		};
		static const unsigned char m[] = {
		    0x0f, 0xc0, 0x1f, 0xe0, 0x3f, 0xf0, 0x7f, 0xf8,
		    0x7f, 0xf8, 0x3f, 0xf0, 0x1f, 0xe0, 0x8f, 0xc1,
		    0x8f, 0xc7, 0xcf, 0xcf, 0xf7, 0x9c, 0x78, 0x78,
		...
		static const unsigned char d[] = {
		    ...
		    0x00, 0xe6, 0x1f, 0xc6, 0x00, 0x06, 0x00, 0x06,
		...
		static const unsigned char d[] = {
		    0x00, 0x00, 0x7f, 0xfe, 0x7f, 0xfe, 0x00, 0x06,
	    ...
	    if (data != NULL) {
		unsigned char d[512], m[512];
		int i, j;

		memset(d, 0, sizeof (d));
		memset(m, 0, sizeof (m));
		for (i = j = 0; i < 32; i += 2, j += 8) {
		    d[j] = data[i];
		    d[j + 1] = data[i + 1];
		    m[j] = mask[i];
		    m[j + 1] = mask[i + 1];
