...
#endif
#if 1
    //// the macOS version - has the same problem with embedded windows.
    TkRegion srcRgn = TkCreateRegion();
    TkUnionRectWithRegion(&srcRect, srcRgn, srcRgn);
    TkRegion dstRgn = TkCreateRegion();
    TkUnionRectWithRegion(&dstRect, dstRgn, dstRgn);
    TkSubtractRegion(srcRgn, dstRgn, damageRgn);
    TkDestroyRegion(srcRgn);
    TkDestroyRegion(dstRgn);
#endif
    return 1;
}

