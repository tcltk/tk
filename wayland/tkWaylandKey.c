...
static void InitializeXKBKeymap(TkDisplay *dispPtr);
static void CleanupXKBKeymap(TkDisplay *dispPtr);
 ...
 * TkpGetKeySym –
 *
 *   Given a KeyPress or KeyRelease event, map the keycode to a
 *   KeySym using the current keyboard mapping.
 *
 * Results:
 *   Returns the KeySym corresponding to the event, or NoSymbol
 *   if the event can’t be translated.
 *
 * Side effects:
 *   None.
 *
...
KeySym
TkpGetKeySym(
	     TkDisplay *dispPtr,		/* Display on which to map keycode. */
	     XEvent *eventPtr)		/* Description of X event. */
{
	...
	/*
	 * The TkKeyEvent structure (an extension of XKeyEvent)
	 * includes the keysym field populated by our event handler.
	 */
	keysym = kePtr->keycode;  /* On Wayland, we store keysym here. */
    }
 ...
 *    or NULL if no string was generated. The *lengthPtr is filled
 *    in with the byte length of the string.
 *
 * Side effects:
 *    The returned string is stored in a static buffer that is
	     ...
	     Tcl_DString *dsPtr)		/* DString to hold the returned string. */
{
     ...
     * generated by our keyboard handler (potentially through IME
     * composition) and stored in the event structure.
     *
     * The TkKeyEvent structure includes a 'trans_chars' field
     * containing the UTF-8 string.
     */

    if (kePtr->nbytes > 0 && kePtr->trans_chars[0] != '\0') {
	/*
	 * Copy the pre-computed string into the DString.
	 */
	Tcl_DStringAppend(dsPtr, kePtr->trans_chars, kePtr->nbytes);
 ...
 * 
 *   Given a keysym and modifier state, find a keycode that will
 *   generate that keysym and update the state to include any
 *   additional modifiers needed.
 * 
 * Results:
 *    The return value is the keycode that generates the keysym.
 *    The *statePtr is modified to include any modifiers that must
 *    be pressed to generate the keysym (e.g., Shift for uppercase).
 * 
 * Side effects:
 *    None.
 * 
...
int
TkpSetKeycodeAndState(
		      Tk_Window tkwin,		/* Window that will receive event. */
		      KeySym keysym,		/* Keysym to generate. */
		      XEvent *eventPtr)		/* Event structure to modify. */
{
    ...
    /*
     * Use XKB to map the keysym back to a keycode.
     * This is needed for event simulation (e.g., [event generate]).
     */

    if (dispPtr->xkb_state) {
	struct xkb_state *xkb_state = (struct xkb_state *)dispPtr->xkb_state;
	struct xkb_keymap *keymap = xkb_state_get_keymap(xkb_state);
	xkb_keycode_t min_keycode, max_keycode;
	xkb_keycode_t kc;
    
	/*
	 * Search through all keycodes to find one that generates
	 * the desired keysym.
	 */
	...
	for (kc = min_keycode; kc <= max_keycode; kc++) {
	    ...
	    num_levels = xkb_keymap_num_levels_for_key(keymap, kc, 0);
		...
		if (num_syms > 0 && syms[0] == keysym) {
		    keycode = kc;
                
		    /*
		     * Determine what modifiers are needed for this level.
	...
	if (keycode == 0) {
	    /*
	     * Keysym not found in keymap. Use a dummy keycode.
	     */
	    keycode = 255;
	}
    } else {
	/*
	 * No XKB state available. Use a simple fallback mapping.
	 */
	if (keysym >= ‘A’ && keysym <= ‘Z’) {
	    keycode = keysym - ‘A’ + 38;  /* QWERTY mapping. */
	    state = ShiftMask;
...
{
    /*
     * Initialize or reinitialize the XKB keymap.
     */
    InitializeXKBKeymap(dispPtr);


    /*
     * Determine which modifiers correspond to mode_switch, num_lock, etc.
     * This information is used by the binding system.
     */
	...
	/*
	 * Find the modifier index for Mode_switch (typically Mod5/AltGr).
	 */
	dispPtr->modeModMask = 0;
    
	xkb_mod_index_t mod_index = xkb_keymap_mod_get_index(
							     keymap, XKB_MOD_NAME_MODE);
	if (mod_index != XKB_MOD_INVALID) {
	    dispPtr->modeModMask = (1 << mod_index);
	}
    
	/*
	 * Find the modifier for Num_Lock.
	 */
	dispPtr->numLockMask = 0;
    
	mod_index = xkb_keymap_mod_get_index(keymap, XKB_MOD_NAME_NUM);
	if (mod_index != XKB_MOD_INVALID) {
	    dispPtr->numLockMask = (1 << mod_index);
	}
    
	/*
	 * Find Alt modifier (typically Mod1).
	 */
	dispPtr->altModMask = Mod1Mask;
    
	/*
	 * Find Meta modifier (varies by keymap).
	 */
	dispPtr->metaModMask = 0;
    
	mod_index = xkb_keymap_mod_get_index(keymap, "Meta");
	if (mod_index != XKB_MOD_INVALID) {
	    dispPtr->metaModMask = (1 << mod_index);
	}
    } else {
	/*
	 * Fallback to default modifier masks if XKB is unavailable.
	 */
	dispPtr->modeModMask = Mod5Mask;
	dispPtr->numLockMask = Mod2Mask;
	dispPtr->altModMask = Mod1Mask;
	dispPtr->metaModMask = 0;
    }
 ...
 * InitializeXKBKeymap –
 *
 * Initializes the XKB keyboard mapping state for a display.
 * 
 * Results:
 *    None.
 * 
 * Side effects:
 *    Allocates XKB context and state structures in dispPtr.
 * 
 *–––––––––––––––––––––––––––––––––––
 */
		    ...
		    TkDisplay *dispPtr)
{
    ...
    /*
     * Create a keymap from the current system keymap.
     */
    ...
    /*
     * Create an XKB state to track modifier states.
     */
    ...
    /*
     * Store the XKB structures in the display.
     */
    dispPtr->xkb_context = xkb_context;
    dispPtr->xkb_keymap = keymap;
    dispPtr->xkb_state = xkb_state;
 ...
 * CleanupXKBKeymap –
 * 
 * Cleans up XKB resources for a display.
 * 
 * Results:
 *     None.
 * 
 * Side effects:
 *     Frees XKB context, keymap, and state structures.
 * 
 *–––––––––––––––––––––––––––––––––––
 */
...
{
    if (dispPtr->xkb_state) {
	xkb_state_unref((struct xkb_state *)dispPtr->xkb_state);
	dispPtr->xkb_state = NULL;
    }


    if (dispPtr->xkb_keymap) {
	xkb_keymap_unref((struct xkb_keymap *)dispPtr->xkb_keymap);
	dispPtr->xkb_keymap = NULL;
    }

    if (dispPtr->xkb_context) {
	xkb_context_unref((struct xkb_context *)dispPtr->xkb_context);
	dispPtr->xkb_context = NULL;
    }
