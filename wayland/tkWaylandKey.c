 ...
 *   or NULL if no string was generated.
 *
 * Side effects:
 *   The returned string is stored in a DString.
 * 
 *---------------------------------------------------------------------------
 */
...
const char *
TkpGetString(
    TCL_UNUSED(TkWindow *),      /* Window where event occurred. */
 ...
 * 
 * Results:
 *   None (void function as per Tk API).
 * 
 * Side effects:
 *   Updates the event structure with keycode and state.
 * 
 *---------------------------------------------------------------------------
 */
...
void
TkpSetKeycodeAndState(
    TCL_UNUSED(Tk_Window),       /* Window that will receive event. */
    ...
    TkDisplay *dispPtr)
{
    ...
    dispPtr->metaModMask = 0;           /* Meta (not commonly used) */
