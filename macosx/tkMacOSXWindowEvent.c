...
/*Restrict event processing to VirtualEvents.*/
static Tk_RestrictAction
VirtualEventRestrictProc(
    ClientData arg,
    XEvent *eventPtr)
{
    return (eventPtr->type==VirtualEvent ? TK_PROCESS_EVENT : TK_DEFER_EVENT);
}
...
- (void) generateExposeEvents: (HIShapeRef) shape
{
	 ...
	 * were created when the expose events were processed. To prevent hangs
	 * when virtual events are generated by one of the idle processes we
	 * also process virtual events here.  Unfortunately this does not work
	 * in macOS 10.13.
	 */
	if ([self inLiveResize]) {
	    while (Tcl_DoOneEvent(TCL_IDLE_EVENTS)) {}
	} else if ([NSApp macMinorVersion] > 13) {
	    oldProc = Tk_RestrictEvents(VirtualEventRestrictProc, NULL, &oldArg);
