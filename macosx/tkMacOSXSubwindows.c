 ...
 *
 *	Invokes windowActivation() as a handler for the
 *	NSWindowDidResignKeyNotification, which is generated by the orderOut
 *	method if the unmapped window is the key window. windowActivation() also
 *	handles the assignment of a new key window and ordering it front.
 *
 *----------------------------------------------------------------------
 */
    ...
    Window window)		/* Window. */
{
    ...
    /*
     * Beware that the orderOut method triggers execution of the
     * windowActivation() event handler if the window being unmapped is
     * the key window, invoking TkMacOSXAssignNewKeyWindow() in turn.
     * TkMacOSXAssignNewKeyWindow() requires that the TK_MAPPED bit in
     * flags has been updated, so we need to do that here before calling
     * orderOut.
     */
	...
	if (!Tk_IsEmbedded(winPtr) &&
	    ...
	    [win orderOut:NSApp]; /* Note: side effects described above. */
	}
