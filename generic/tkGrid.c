...
typedef struct Gridder {
    ...
    struct Gridder *gridderPtr;	/* Container window within which this window is
    ...
    struct Gridder *contentPtr;	/* First in list of content windows managed inside this
				 * container (NULL means no grid content). */
...
static int		CheckSlotData(Gridder *gridderPtr, int slot,
...
static void		InitContainerData(Gridder *gridder);
...
static int		SetColumn(Tcl_Interp *interp, Gridder *contentPtr,
			    int column, int numCols);
static int		SetRow(Tcl_Interp *interp, Gridder *contentPtr,
			    int row, int numRows);
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    gridderPtr = GetGrid(container);

    if (objc == 3) {
	gridPtr = gridderPtr->containerDataPtr;
	...
	if (gridderPtr->abortPtr != NULL) {
	    *gridderPtr->abortPtr = 1;
	}
	if (!(gridderPtr->flags & REQUESTED_RELAYOUT)) {
	    gridderPtr->flags |= REQUESTED_RELAYOUT;
	    Tcl_DoWhenIdle(ArrangeGrid, gridderPtr);
	}
    }
    return TCL_OK;
}
...
{
    Tk_Window container;
    Gridder *gridderPtr;		/* container grid record */
	...
	Tcl_WrongNumArgs(interp, 2, objv, "container ?column row ?column row??");
	return TCL_ERROR;
    }
	...
	if (contentPtr->gridderPtr != NULL) {
		...
		if (contentPtr->gridderPtr != NULL) {
		    contentPtr->in = Tcl_NewStringObj(
			    Tk_PathName(contentPtr->gridderPtr->tkwin), -1);
		    Tcl_IncrRefCount(contentPtr->in);
		}
	    }
	    Tk_ManageGeometry(content, NULL, NULL);
	    if (contentPtr->gridderPtr->tkwin != Tk_Parent(contentPtr->tkwin)) {
		Tk_UnmaintainGeometry(contentPtr->tkwin,
			contentPtr->gridderPtr->tkwin);
	    }
	    Unlink(contentPtr);
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    Tcl_DictObjPut(NULL, infoObj, Tcl_NewStringObj("-in", -1),
	    TkNewWindowObj(contentPtr->gridderPtr->tkwin));
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    if (objc != 5) {
	Tcl_WrongNumArgs(interp, 2, objv, "container x y");
	return TCL_ERROR;
    }
    ...
    gridderPtr = GetGrid(container);
    if (gridderPtr->containerDataPtr == NULL) {
    ...
    while (gridderPtr->flags & REQUESTED_RELAYOUT) {
	Tcl_CancelIdleCall(ArrangeGrid, gridderPtr);
	ArrangeGrid(gridderPtr);
    }
    SetGridSize(gridderPtr);
    endX = MAX(gridPtr->columnEnd, gridPtr->columnMax);
    endY = MAX(gridPtr->rowEnd, gridPtr->rowMax);

    slotPtr = gridderPtr->containerDataPtr->columnPtr;
    if (x < gridderPtr->containerDataPtr->startX) {
	i = -1;
    } else {
	x -= gridderPtr->containerDataPtr->startX;
	for (i = 0; slotPtr[i].offset < x && i < endX; i++) {
	    /* null body */
	}
    }

    slotPtr = gridderPtr->containerDataPtr->rowPtr;
    if (y < gridderPtr->containerDataPtr->startY) {
	j = -1;
    } else {
	y -= gridderPtr->containerDataPtr->startY;
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
	...
	Tcl_SetObjResult(interp,
		Tcl_NewBooleanObj(!(gridderPtr->flags & DONT_PROPAGATE)));
	return TCL_OK;
    }
    ...
    old = !(gridderPtr->flags & DONT_PROPAGATE);
    if (propagate != old) {
	if (propagate) {
	    /*
	    ...
	    if (gridderPtr->contentPtr != NULL) {
		if (TkSetGeometryContainer(interp, container, "grid")	!= TCL_OK) {
		    return TCL_ERROR;
		}
		gridderPtr->flags |= ALLOCED_CONTAINER;
	    }
	    gridderPtr->flags &= ~DONT_PROPAGATE;
	} else {
	    if (gridderPtr->flags & ALLOCED_CONTAINER) {
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
	...
	Tcl_WrongNumArgs(interp, 2, objv, "container index ?-option value ...?");
	return TCL_ERROR;
    }
	...
	ok = CheckSlotData(gridderPtr, slot, slotType, /* checkOnly */ 1);
	if (ok == TCL_OK) {
	    slotPtr = (slotType == COLUMN) ?
		    gridderPtr->containerDataPtr->columnPtr :
		    gridderPtr->containerDataPtr->rowPtr;
	}
	...
	if (Tcl_GetIntFromObj(NULL, lObjv[j], &slot) == TCL_OK) {
	    ...
	    contentPtr = gridderPtr->contentPtr;
	    if (contentPtr == NULL) {
		continue;
	    }
	    allContent = 1;
	    ...
	    contentPtr = GetGrid(content);
	    if (contentPtr->gridderPtr != gridderPtr) {
		Tcl_SetObjResult(interp, Tcl_ObjPrintf(
			"the window \"%s\" is not managed by \"%s\"",
			Tcl_GetString(lObjv[j]), Tcl_GetString(objv[2])));
		Tcl_SetErrorCode(interp, "TK", "GRID", "NOT_CONTAINER", NULL);
		Tcl_DecrRefCount(listCopy);
		return TCL_ERROR;
	    }
	} else {
	...
	do {
		...
		ok = CheckSlotData(gridderPtr, slot, slotType, /*checkOnly*/ 0);
		if (ok != TCL_OK) {
		    Tcl_SetObjResult(interp, Tcl_ObjPrintf(
			    "\"%s\" is out of range",
    ...
    if (slotPtr != NULL) {
	if (slotType == ROW) {
	    int last = gridderPtr->containerDataPtr->rowMax - 1;
	    ...
	    gridderPtr->containerDataPtr->rowMax = last+1;
	} else {
	    int last = gridderPtr->containerDataPtr->columnMax - 1;
	    ...
	    gridderPtr->containerDataPtr->columnMax = last + 1;
	}
    }
 ...
 *
 * Side effects:
 *	Places a list of contents of the specified window in the interpreter's
 *	result field.
 *
 *----------------------------------------------------------------------
 */
    ...
    Tcl_Obj *const objv[])	/* Argument objects. */
{
    ...
    for (contentPtr = gridderPtr->contentPtr; contentPtr != NULL;
	    contentPtr = contentPtr->nextPtr) {
...
{
    register Gridder *contentPtr = clientData;

    if (contentPtr->gridderPtr->tkwin != Tk_Parent(contentPtr->tkwin)) {
	Tk_UnmaintainGeometry(contentPtr->tkwin, contentPtr->gridderPtr->tkwin);
    }
    Unlink(contentPtr);
    Tk_UnmapWindow(contentPtr->tkwin);
}
 ...
 *
 * Side effects:
 *	The content of gridderPtr may get resized or moved.
 *
 *----------------------------------------------------------------------
 */
...
{
    register Gridder *gridderPtr = clientData;
    register Gridder *contentPtr;
    GridContainer *slotPtr = gridderPtr->containerDataPtr;
    int abort;
	...
	*gridderPtr->abortPtr = 1;
    ...
    gridderPtr->abortPtr = &abort;
    abort = 0;
    Tcl_Preserve(gridderPtr);
    ...
    SetGridSize(gridderPtr);
    width = ResolveConstraints(gridderPtr, COLUMN, 0);
    height = ResolveConstraints(gridderPtr, ROW, 0);
    width += Tk_InternalBorderLeft(gridderPtr->tkwin) +
	    Tk_InternalBorderRight(gridderPtr->tkwin);
    height += Tk_InternalBorderTop(gridderPtr->tkwin) +
	    Tk_InternalBorderBottom(gridderPtr->tkwin);

    if (width < Tk_MinReqWidth(gridderPtr->tkwin)) {
	width = Tk_MinReqWidth(gridderPtr->tkwin);
    }
    if (height < Tk_MinReqHeight(gridderPtr->tkwin)) {
	height = Tk_MinReqHeight(gridderPtr->tkwin);
    }

    if (((width != Tk_ReqWidth(gridderPtr->tkwin))
	    || (height != Tk_ReqHeight(gridderPtr->tkwin)))
	    && !(gridderPtr->flags & DONT_PROPAGATE)) {
	Tk_GeometryRequest(gridderPtr->tkwin, width, height);
	if (width>1 && height>1) {
	    gridderPtr->flags |= REQUESTED_RELAYOUT;
	    Tcl_DoWhenIdle(ArrangeGrid, gridderPtr);
	}
	gridderPtr->abortPtr = NULL;
	Tcl_Release(gridderPtr);
	return;
    }
    ...
    realWidth = Tk_Width(gridderPtr->tkwin) -
	    Tk_InternalBorderLeft(gridderPtr->tkwin) -
	    Tk_InternalBorderRight(gridderPtr->tkwin);
    realHeight = Tk_Height(gridderPtr->tkwin) -
	    Tk_InternalBorderTop(gridderPtr->tkwin) -
	    Tk_InternalBorderBottom(gridderPtr->tkwin);
    usedX = AdjustOffsets(realWidth,
	    MAX(slotPtr->columnEnd, slotPtr->columnMax), slotPtr->columnPtr);
    usedY = AdjustOffsets(realHeight, MAX(slotPtr->rowEnd, slotPtr->rowMax),
	    slotPtr->rowPtr);
    TkComputeAnchor(gridderPtr->containerDataPtr->anchor, gridderPtr->tkwin,
    ...
    for (contentPtr = gridderPtr->contentPtr; contentPtr != NULL && !abort;
	    contentPtr = contentPtr->nextPtr) {
	...
	if (gridderPtr->tkwin == Tk_Parent(contentPtr->tkwin)) {
	    if ((width <= 0) || (height <= 0)) {
		Tk_UnmapWindow(contentPtr->tkwin);
	    } else {
		...
		if (Tk_IsMapped(gridderPtr->tkwin)) {
		    Tk_MapWindow(contentPtr->tkwin);
		}
	    }
	} else if ((width <= 0) || (height <= 0)) {
	    Tk_UnmaintainGeometry(contentPtr->tkwin, gridderPtr->tkwin);
	    Tk_UnmapWindow(contentPtr->tkwin);
	} else {
	    Tk_MaintainGeometry(contentPtr->tkwin, gridderPtr->tkwin, x, y,
		    width, height);
	}
    }
...
static int
ResolveConstraints(
    Gridder *gridderPtr,		/* The geometry container for this grid. */
				 ...
				 * pixels, or 0 (not currently used). */
{
    ...
    register Gridder *contentPtr;	/* List of content windows in this grid. */
    ...
    if (slotType == COLUMN) {
	constraintCount = gridderPtr->containerDataPtr->columnMax;
	slotCount = gridderPtr->containerDataPtr->columnEnd;
	slotPtr = gridderPtr->containerDataPtr->columnPtr;
    } else {
	constraintCount = gridderPtr->containerDataPtr->rowMax;
	slotCount = gridderPtr->containerDataPtr->rowEnd;
	slotPtr = gridderPtr->containerDataPtr->rowPtr;
    }
...
SetGridSize(
    Gridder *gridderPtr)		/* The geometry container for this grid. */
{
    register Gridder *contentPtr;	/* Current content window. */
    int maxX = 0, maxY = 0;
    ...
    gridderPtr->containerDataPtr->columnEnd = maxX;
    gridderPtr->containerDataPtr->rowEnd = maxY;
    CheckSlotData(gridderPtr, maxX, COLUMN, CHECK_SPACE);
    CheckSlotData(gridderPtr, maxY, ROW, CHECK_SPACE);
}

/*
 *----------------------------------------------------------------------
    ...
    int checkOnly)		/* Don't allocate new space if true. */
{
    ...
    if ((checkOnly == CHECK_ONLY) && (gridderPtr->containerDataPtr == NULL)) {
	return TCL_ERROR;
    }
    ...
    InitContainerData(gridderPtr);
    end = (slotType == ROW) ? gridderPtr->containerDataPtr->rowMax :
	    gridderPtr->containerDataPtr->columnMax;
    if (checkOnly == CHECK_ONLY) {
    	return ((end < slot) ? TCL_ERROR : TCL_OK);
    } else {
    	numSlot = (slotType == ROW) ? gridderPtr->containerDataPtr->rowSpace
		: gridderPtr->containerDataPtr->columnSpace;
    	if (slot >= numSlot) {
	    ...
	    ckfree(oldSI);
	    if (slotType == ROW) {
	 	gridderPtr->containerDataPtr->rowPtr = newSI;
	    	gridderPtr->containerDataPtr->rowSpace = newNumSlot;
	    } else {
	    	gridderPtr->containerDataPtr->columnPtr = newSI;
	    	gridderPtr->containerDataPtr->columnSpace = newNumSlot;
	    }
	}
	if (slot >= end && checkOnly != CHECK_SPACE) {
	    if (slotType == ROW) {
		gridderPtr->containerDataPtr->rowMax = slot+1;
	    } else {
		gridderPtr->containerDataPtr->columnMax = slot+1;
	    }
	}
    	return TCL_OK;
    }
...
{
    if (gridderPtr->containerDataPtr == NULL) {
	GridContainer *gridPtr = gridderPtr->containerDataPtr =
...
static void
Unlink(
    register Gridder *contentPtr)	/* Window to unlink. */
{
    register Gridder *gridderPtr, *contentPtr2;

    gridderPtr = contentPtr->gridderPtr;
    if (gridderPtr == NULL) {
	return;
    }

    if (gridderPtr->contentPtr == contentPtr) {
	gridderPtr->contentPtr = contentPtr->nextPtr;
    } else {
	for (contentPtr2=gridderPtr->contentPtr ; ; contentPtr2=contentPtr2->nextPtr) {
	    if (contentPtr2 == NULL) {
	...
	*gridderPtr->abortPtr = 1;
    ...
    SetGridSize(contentPtr->gridderPtr);
    contentPtr->gridderPtr = NULL;
    ...
    if ((gridderPtr->contentPtr == NULL) && (gridderPtr->flags & ALLOCED_CONTAINER)) {
	TkFreeGeometryContainer(gridderPtr->tkwin, "grid");
	gridderPtr->flags &= ~ALLOCED_CONTAINER;
    }
}

    ...
    XEvent *eventPtr)		/* Describes what just happened. */
{
	...
	if ((gridPtr->gridderPtr != NULL) &&
		(gridPtr->doubleBw != 2*Tk_Changes(gridPtr->tkwin)->border_width)) {
	    if (!(gridPtr->gridderPtr->flags & REQUESTED_RELAYOUT)) {
		gridPtr->doubleBw = 2*Tk_Changes(gridPtr->tkwin)->border_width;
		gridPtr->gridderPtr->flags |= REQUESTED_RELAYOUT;
		Tcl_DoWhenIdle(ArrangeGrid, gridPtr->gridderPtr);
	    }
	}
    } else if (eventPtr->type == DestroyNotify) {
	register Gridder *contentPtr, *nextPtr;
				 ...
				 * that there is at least one window name. */
{
    ...
    if (defaultRow < 0) {
	if (gridderPtr != NULL && gridderPtr->containerDataPtr != NULL) {
	    SetGridSize(gridderPtr);
	    defaultRow = gridderPtr->containerDataPtr->rowEnd;
	} else {
	    defaultRow = 0;
	}
    }
	...
	for (i = numWindows; i < objc; i += 2) {
		...
		if (SetColumn(interp, contentPtr, tmp, -1) != TCL_OK) {
		    return TCL_ERROR;
		}
		break;
		...
		if (SetColumn(interp, contentPtr, -1, tmp) != TCL_OK) {
		    return TCL_ERROR;
		}
		break;
	    case CONF_IN:
		...
		if (SetRow(interp, contentPtr, tmp, -1) != TCL_OK) {
		    return TCL_ERROR;
		}
		break;
	    case CONF_ROWSPAN:
		...
		if (SetRow(interp, contentPtr, -1, tmp) != TCL_OK) {
		    return TCL_ERROR;
		}
		break;
	    }
	 ...
	 */
    	...
    	if (!positionGiven && (contentPtr->gridderPtr != NULL)) {
	    gridderPtr = contentPtr->gridderPtr;
	    goto scheduleLayout;
    	}
	...
	if (positionGiven && (gridderPtr == contentPtr->gridderPtr)) {
	    goto scheduleLayout;
	}
	...
	if (contentPtr->gridderPtr != NULL && contentPtr->gridderPtr != gridderPtr) {
            if (contentPtr->gridderPtr->tkwin != Tk_Parent(contentPtr->tkwin)) {
                Tk_UnmaintainGeometry(contentPtr->tkwin, contentPtr->gridderPtr->tkwin);
            }
	    Unlink(contentPtr);
	...
	if (contentPtr->gridderPtr == NULL) {
	    Gridder *tempPtr = gridderPtr->contentPtr;

	    contentPtr->gridderPtr = gridderPtr;
	    gridderPtr->contentPtr = contentPtr;
	...
	for (ancestor = gridderPtr->tkwin; ; ancestor = Tk_Parent(ancestor)) {
	    if (ancestor == parent) {
		break;
	    }
	...
	for (container = (TkWindow *)gridderPtr->tkwin; container != NULL;
	     container = (TkWindow *)TkGetGeomContainer(container)) {
	    if (container == (TkWindow *)content) {
		Tcl_SetObjResult(interp, Tcl_ObjPrintf(
		    "can't put %s inside %s, would cause management loop",
	            Tcl_GetString(objv[j]), Tk_PathName(gridderPtr->tkwin)));
		Tcl_SetErrorCode(interp, "TK", "GEOMETRY", "LOOP", NULL);
		Unlink(contentPtr);
		return TCL_ERROR;
	    }
	}
	if (gridderPtr->tkwin != Tk_Parent(content)) {
	    ((TkWindow *)content)->maintainerPtr = (TkWindow *)gridderPtr->tkwin;
	}

	Tk_ManageGeometry(content, &gridMgrType, contentPtr);

	if (!(gridderPtr->flags & DONT_PROPAGATE)) {
	    if (TkSetGeometryContainer(interp, gridderPtr->tkwin, "grid")
		    != TCL_OK) {
		Tk_ManageGeometry(content, NULL, NULL);
		Unlink(contentPtr);
		return TCL_ERROR;
	...
	if (contentPtr->column == -1) {
	    if (SetColumn(interp, contentPtr, defaultColumn,-1) != TCL_OK){
		return TCL_ERROR;
	    }
	}
	if (SetColumn(interp, contentPtr, -1,
		contentPtr->numCols + defaultColumnSpan - 1) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (contentPtr->row == -1) {
	    if (SetRow(interp, contentPtr, defaultRow, -1) != TCL_OK) {
		return TCL_ERROR;
	    }
	}
	...
	if (gridderPtr == NULL) {
	    Tcl_SetObjResult(interp, Tcl_NewStringObj(
		    "can't use '^', can't find container", -1));
	    Tcl_SetErrorCode(interp, "TK", "GRID", "SHORTCUT_USAGE", NULL);
	    return TCL_ERROR;
	}
	...
	if (!match) {
	    Tcl_SetObjResult(interp, Tcl_NewStringObj(
		    "can't find window to extend with \"^\"", -1));
	    Tcl_SetErrorCode(interp, "TK", "GRID", "SHORTCUT_USAGE", NULL);
	    return TCL_ERROR;
	}
    }
