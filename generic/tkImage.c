    ...
    Tcl_Obj *const objv[])	/* Argument strings. */
{
	...
	if ((objc == 3) || (*(arg = Tcl_GetString(objv[3])) == '-')) {
	     ...
	     * the name of the current main widget command (normally "." but
	...
	if (isNew) {
	    modelPtr = ckalloc(sizeof(ImageModel));
	    ...
	    modelPtr = Tcl_GetHashValue(hPtr);
    ...
    Tk_ImageModel imageModel)	/* Token for image. */
{
    ...
    return Tcl_GetHashKey(modelPtr->tablePtr, modelPtr->hPtr);
}

/*
 *----------------------------------------------------------------------
