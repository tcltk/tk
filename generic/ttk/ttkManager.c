 ...
 * it recomputes the size and position of all content and places them.
 *
 ...
 *
 * +++ Content removal <<NOTE-LOSTCONTENT>>.
 *
 * There are three conditions under which a content is removed:
 *
 * (1) Another GM claims control
 * (2) Manager voluntarily relinquishes control
 * (3) Content is destroyed
 *
 * In case (1), Tk calls the manager's lostContentProc.
 * Case (2) is performed by calling Tk_ManageGeometry(content,NULL,0);
 ...
 * register an event handler on the content widget to track <Destroy> events.
 */
...
/* content->flags bits:
 */
#define CONTENT_MAPPED 		0x1	/* content to be mapped when container is */
...
/* ++ RecomputeLayout --
 * 	Recompute geometry of all content.
 */
static void RecomputeLayout(Ttk_Manager *mgr)
{
    mgr->managerSpec->Place(mgr->managerData);
...
static void ManagerEventHandler(ClientData clientData, XEvent *eventPtr)
{
	...
	case MapNotify:
		...
		if (content->flags & CONTENT_MAPPED) {
		    Tk_MapWindow(content->contentWindow);
		}
	    }
	    break;
	case UnmapNotify:
	    for (i = 0; i < mgr->nContent; ++i) {
		Ttk_Content *content = mgr->content[i];
		Tk_UnmapWindow(content->contentWindow);
	    }
	    break;
    }
}
...
/* ++ ContentEventHandler --
 * 	Notifies manager when a content is destroyed
 * 	(see <<NOTE-LOSTCONTENT>>).
 */
static void ContentEventHandler(ClientData clientData, XEvent *eventPtr)
{
    Ttk_Content *content = (Ttk_Content *)clientData;
    if (eventPtr->type == DestroyNotify) {
	content->manager->managerSpec->tkGeomMgr.lostContentProc(
	    content->manager, content->contentWindow);
    }
}
...
static Ttk_Content *NewContent(
    Ttk_Manager *mgr, Tk_Window contentWindow, void *clientData)
{
    Ttk_Content *content = (Ttk_Content *)ckalloc(sizeof(*content));
...
static void InsertContent(Ttk_Manager *mgr, Ttk_Content *content, int index)
{
    ...
    Tk_CreateEventHandler(content->contentWindow,
	    StructureNotifyMask, ContentEventHandler, (ClientData)content);

    ScheduleUpdate(mgr, MGR_RESIZE_REQUIRED);
}

/* RemoveContent --
 * 	Unmanage and delete the content.
 *
 * NOTES/ASSUMPTIONS:
 *
 * [1] It's safe to call Tk_UnmapWindow / Tk_UnmaintainGeometry even if this
 * routine is called from the content's DestroyNotify event handler.
 */
static void RemoveContent(Ttk_Manager *mgr, int index)
{
     ...
     */
    Tk_DeleteEventHandler(
	content->contentWindow, StructureNotifyMask, ContentEventHandler, content);

    /* Note [1] */
    Tk_UnmaintainGeometry(content->contentWindow, mgr->containerWindow);
    Tk_UnmapWindow(content->contentWindow);
...
void Ttk_InsertContent(
    Ttk_Manager *mgr, int index, Tk_Window tkwin, void *contentData)
{
    Ttk_Content *content = NewContent(mgr, tkwin, contentData);
    InsertContent(mgr, content, index);
}

/* ++ Ttk_ForgetContent --
 * 	Unmanage the specified content.
 */
...
{
    Ttk_Content *content = mgr->content[index];
    Tk_MaintainGeometry(content->contentWindow,mgr->containerWindow,x,y,width,height);
...
/* ++ Ttk_UnmapContent --
 * 	Unmap the specified content, but leave it managed.
 */
...
{
    return mgr->content[index]->clientData;
}
 ...
 * 	Return the index of the content specified by objPtr.
 * 	Content may be specified as an integer index or
    ...
    Tcl_Interp *interp, Ttk_Manager *mgr, Tcl_Obj *objPtr, int *indexPtr)
{
     ...
     */
    if (Tcl_GetIntFromObj(NULL, objPtr, &contentIndex) == TCL_OK) {
	if (contentIndex < 0 || contentIndex >= mgr->nContent) {
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf(
		"index %d out of bounds", contentIndex));
	    Tcl_SetErrorCode(interp, "TTK", "CONTENT", "INDEX", NULL);
	    return TCL_ERROR;
	}
	*indexPtr = contentIndex;
	return TCL_OK;
    }
	...
	contentIndex = Ttk_ContentIndex(mgr, tkwin);
	if (contentIndex < 0) {
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf(
		    "%s is not managed by %s", string,
		    Tk_PathName(mgr->containerWindow)));
	    Tcl_SetErrorCode(interp, "TTK", "CONTENT", "MANAGER", NULL);
	    return TCL_ERROR;
	}
	*indexPtr = contentIndex;
	return TCL_OK;
    ...
    Tcl_SetObjResult(interp, Tcl_ObjPrintf(
	    "Invalid content specification %s", string));
    Tcl_SetErrorCode(interp, "TTK", "CONTENT", "SPEC", NULL);
    return TCL_ERROR;
}
 ...
 */
int Ttk_Maintainable(Tcl_Interp *interp, Tk_Window content, Tk_Window container)
{
    Tk_Window ancestor = container, parent = Tk_Parent(content);

    if (Tk_IsTopLevel(content) || content == container) {
	goto badWindow;
    }
...
badWindow:
    Tcl_SetObjResult(interp, Tcl_ObjPrintf("can't add %s as content of %s",
	    Tk_PathName(content), Tk_PathName(container)));
