...
static void		ContainerStructureProc(ClientData clientData,
			    XEvent *eventPtr);
    ...
    Tk_OptionTable table)
{
    ...
    contentPtr = ckalloc(sizeof(Content));
    ...
    Tk_Window tkwin)		/* Token for desired content. */
{
    ...
    hPtr = Tcl_FindHashEntry(&dispPtr->contentTable, (char *) tkwin);
    if (hPtr == NULL) {
	return NULL;
    }
    return Tcl_GetHashValue(hPtr);
...
{
    register Container *containerPtr;
    register Content *prevPtr;
    ...
    Tk_Window tkwin)		/* Token for desired container. */
{
	...
	Tk_CreateEventHandler(containerPtr->tkwin, StructureNotifyMask,
		ContainerStructureProc, containerPtr);
    } else {
    ...
    Tk_Window tkwin)		/* Token for desired container. */
{
    ...
    hPtr = Tcl_FindHashEntry(&dispPtr->containerTable, (char *) tkwin);
    if (hPtr == NULL) {
	return NULL;
    }
    return Tcl_GetHashValue(hPtr);
    ...
    Tcl_Obj *const objv[])	/* Object values for arguments. */
{
	...
	for (container = (TkWindow *)tkwin; container != NULL;
	     container = (TkWindow *)TkGetGeomContainer(container)) {
...
{
    register Container *containerPtr = clientData;
    register Content *contentPtr;
...
{
    register Container *containerPtr = clientData;
    register Content *contentPtr, *nextPtr;
...
ContentStructureProc(
    ...
    XEvent *eventPtr)		/* Describes what just happened. */
{
    register Content *contentPtr = clientData;
 ...
 *
 * Side effects:
 *	Forgets all placer-related information about the content.
 *
 *--------------------------------------------------------------
 */
