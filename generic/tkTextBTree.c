...
static int		CharDeleteProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr, int flags);
...
static int		HyphenDeleteProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr, int flags);
...
static void		RebuildSections(TkSharedText *sharedTextPtr, TkTextLine *linePtr,
			    int propagateChangeOfNumBranches);
static int		CheckSegments(const TkSharedText *sharedTextPtr, const TkTextLine *linePtr);
static int		CheckSections(const TkTextLine *linePtr);
static int		CheckSegmentItems(const TkSharedText *sharedTextPtr, const TkTextLine *linePtr);
...
static int		BranchDeleteProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr, int flags);
static int		BranchRestoreProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr);
...
static int		LinkDeleteProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr, int flags);
static int		LinkRestoreProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr);
static Tcl_Obj *	LinkInspectProc(const TkSharedText *sharedTextPtr, const TkTextSegment *segPtr);
static int		ProtectionMarkDeleteProc(TkSharedText *sharedTextPtr, TkTextSegment *segPtr,
			    int flags);
...
static int		UndoIndexIsEqual(const TkTextUndoIndex *indexPtr1,
...
static void		ReInsertSegment(const TkSharedText *sharedTextPtr,
			    const TkTextUndoIndex *indexPtr, TkTextSegment *segPtr, int updateNode);

/*
...
static void UndoTagPerform(TkSharedText *, TkTextUndoInfo *, TkTextUndoInfo *, int);
static void UndoTagDestroy(TkSharedText *, TkTextUndoToken *token, int);
static Tcl_Obj *UndoTagGetCommand(const TkSharedText *, const TkTextUndoToken *);

static void UndoClearTagsPerform(TkSharedText *, TkTextUndoInfo *, TkTextUndoInfo *, int);
static void RedoClearTagsPerform(TkSharedText *, TkTextUndoInfo *, TkTextUndoInfo *, int);
static void UndoClearTagsDestroy(TkSharedText *, TkTextUndoToken *, int);
...
static void UndoDeletePerform(TkSharedText *, TkTextUndoInfo *, TkTextUndoInfo *, int);
static void RedoDeletePerform(TkSharedText *, TkTextUndoInfo *, TkTextUndoInfo *, int);
static void UndoDeleteDestroy(TkSharedText *, TkTextUndoToken *, int);
...
static void UndoInsertPerform(TkSharedText *, TkTextUndoInfo *, TkTextUndoInfo *, int);
...
#define POINTER_IS_MARKED(ptr)	((int)(((__ptr_to_int *) &ptr)->flag & (uintptr_t) 1))
    ...
    TCL_UNUSED(int))
{
	...
	if (segPtr->typePtr->restoreProc && !segPtr->typePtr->restoreProc(sharedTextPtr, segPtr)) {
	    ...
	    if (!firstPtr) {
		/*
		 * The original first segment(s) of the token were dropped:
		 * move the split point protection to the first restored
		 * segment, the final cleanup still has to merge the left
		 * boundary of the restored range.
		 */
		firstPtr = segPtr;
		firstPtr->protectionFlag = 1;
	    }
	...
	if (!TkTextTagSetTest(nodePtr->tagoffPtr, i)) {
	    AddTagToNode(nodePtr, sharedTextPtr->tagLookup[i], 1);
	}
    }
	...
	AddTagToNode(nodePtr, sharedTextPtr->tagLookup[i], 0);
    }
    ...
    TCL_UNUSED(int))
{
    ...
    if (updateElideInfo) {
	/*
	 * Recompute the elide topology of the restored range from the
	 * current tag information, extended to the boundaries of the
	 * enclosing elided ranges (see UndoDeletePerform). The range ends
	 * have to be protected (contract of UpdateElideInfo).
	 */

	TkTextSegment *fPtr = firstSegPtr;
	TkTextSegment *lPtr = lastSegPtr;
	TkTextSegment *predPtr = GetPrevTagInfoSegment(firstSegPtr);
	TkTextSegment *succPtr;
	TkTextLine *nextLinePtr;

	if (lastSegPtr->nextPtr) {
	    succPtr = GetNextTagInfoSegment(lastSegPtr->nextPtr);
	} else if ((nextLinePtr = lastSegPtr->sectionPtr->linePtr->nextPtr)) {
	    succPtr = GetFirstTagInfoSegment(NULL, nextLinePtr);
	} else {
	    succPtr = NULL;
	}
	...
	lPtr->protectionFlag = 1;
	UpdateElideInfo(sharedTextPtr, NULL, &fPtr, &lPtr, ELISION_HAS_BEEN_CHANGED);
	if (fPtr != firstSegPtr && fPtr != lastSegPtr) {
	    CleanupSplitPoint(fPtr, sharedTextPtr);
	}
	if (lPtr != fPtr && lPtr != firstSegPtr && lPtr != lastSegPtr) {
	    CleanupSplitPoint(lPtr, sharedTextPtr);
	}
    }
    firstSegPtr->protectionFlag = 1;
    ...
    /*
     * The restore overwrites the tag information chunk-wise: segment
     * boundaries lying inside a chunk (e.g. introduced by tagging changes
     * made between the clearing and this undo, and overwritten now) may
     * leave equal tagged adjacent char segments behind, which the cleanup
     * of the range ends above cannot reach. Merge over the restored lines.
     */

    {
	TkTextLine *lp = TkTextIndexGetLine(&startIndex);
	TkTextLine *endLp = linePtr->nextPtr;

	for ( ; lp != endLp; lp = lp->nextPtr) {
	    TkTextSegment *sp = lp->segPtr;

	    while (sp) {
		if (sp->typePtr == &tkTextCharType
			&& sp->nextPtr
			&& sp->nextPtr->typePtr == &tkTextCharType
			&& !sp->protectionFlag
			&& !sp->nextPtr->protectionFlag
			&& TkTextTagSetIsEqual(sp->tagInfoPtr, sp->nextPtr->tagInfoPtr)) {
		    sp = JoinCharSegments(sharedTextPtr, sp); /* re-test the merged segment */
		} else {
		    sp = sp->nextPtr;
		}
	    }
    ...
    unsigned byteSize2)
{
	...
	if (myToken2->endIndex.lineIndex == -1) {
	    /* the end of the second chunk is a mark, it survives the join */
	    myToken1->endIndex = myToken2->endIndex;
	    ...
	    /* the second chunk provably lies on a single line */
	    ...
	    /*
	     * The end of the joined range cannot be computed reliably: with
	     * mark based indices (steady marks) the chunks do not tell
	     * whether they span line boundaries. Refuse the join, the
	     * tokens belong to the same undo atom anyway.
	     */
	    return 0;
	}
    ...
    TkTextSegment *segPtr)	/* First segment of this line. */
{
    ...
    newLinePtr->logicalLine = 1;
    newLinePtr->changed = 1;
    ...
    RebuildSections(sharedTextPtr, newLinePtr, 0);
    ...
    LoadData *loadData)	/* Release this data. */
{
	...
	RebuildSections(loadData->sharedTextPtr, loadData->linePtr, 1);
    }
    if (index0 >= 0) {
	if (index1 >= 0) {
	    if (index2 >= 0) {
...
TkBTreeLoad(
    ...
    int validOptions)	/* Do we report errors in case of invalid option values? */
{
    enum {
	STATE_START        = 1 << 0,
	STATE_SETUP        = 1 << 1,
    ...
    TkTextUndoInfo *undoInfo)	/* Undo information, can be NULL. */
{
	...
	for (i = TkTextTagSetFindFirst(tagInfoPtr);
		...
		AddTagToNode(linePtr->parentPtr, sharedTextPtr->tagLookup[i], 1);
	    }
	}
    }
...
TkBTreeReInsertSegment(
    ...
    TkTextSegment *segPtr)
{
    ReInsertSegment(sharedTextPtr, indexPtr, segPtr, 1);
}

/*
 *----------------------------------------------------------------------
...
{
    TkTextSegment *segPtr;
    int isBranchSegment, isLinkSegment;
    int length;
    ...
    TkTextSegment *segPtr)
{
	...
	case TK_TEXT_TAGGING_WITHIN:
	    if (offset > 0) {
		return 1; /* inserting into a char segment */
	    }
    ...
    TkTextUndoInfo *undoInfo)	/* Store undo information, can be NULL. */
{
	...
	if (!segPtr) {
	    ...
	    /*
	     * Save branch or link segment if its counterpart lies outside of the
	     * deletion range, it will be re-added after deletion. This prevents
	     * orphaned branches/links. A link whose branch lies before the range
	     * can only be the first switch inside the range, and a saved branch
	     * either annihilates with the next link, or the range is ending
	     * inside of its elided region.
	     */
		...
		if (segPtr->body.link.prevPtr == savedBranchPtr) {
		     ...
		     */
		    if (undoInfo) {
			/* It's already recorded in the undo chain: record it unlinked. */
			savedBranchPtr->nextPtr = NULL;
		...
		if (segPtr->typePtr->group == SEG_GROUP_BRANCH) {
		    /*
		     * A switch refuses to die only if undo is active (otherwise
		     * DELETE_BRANCHES is set). It leaves the tree now, kept alive
		     * by the undo chain, and by savedBranchPtr/savedLinkPtr if it
		     * will be re-added after this loop: release the reference
		     * held by the tree.
		     */
		    ...
		    /*
		     * This mark refuses to die. Link this segment after prevPtr1.
		     */
	    ...
	    if (savePtr) {
		/* Note that switches (size == 0) misuse tagInfoPtr for their counterpart. */
		if (savePtr->size > 0 && savePtr->tagInfoPtr) {
	...
	TkTextSegment *branchPtr = savedLinkPtr->body.link.prevPtr;
	TkTextSegment *linkPtr = savedBranchPtr->body.branch.nextPtr;
	...
	linkPtr->body.link.prevPtr = branchPtr;
	if (undoInfo) {
	    /* Both are already recorded in the undo chain: record them unlinked. */
	    savedLinkPtr->nextPtr = NULL;
    ...
    RebuildSections(sharedTextPtr, linePtr1, 1);
	...
	if (!TkTextIsSpecialOrPrivateMark(firstSegPtr)
	    ...
	    if (joinPtr && joinPtr->typePtr == &tkTextCharType) {
		/*
		 * The removed boundary may have separated two equal tagged
		 * char segments (e.g. after the deletion merged two elided
		 * ranges, with the split points being the dead switches).
		 */
		CleanupCharSegments(sharedTextPtr, joinPtr);
	    }
	    countChanges += 1;
	}
	if (!TkTextIsSpecialOrPrivateMark(lastSegPtr)
	    ...
	    if (joinPtr && joinPtr->sectionPtr && joinPtr->typePtr == &tkTextCharType) {
		/* See the first boundary above. */
		CleanupCharSegments(sharedTextPtr, joinPtr);
	    }
	    countChanges += 1;
	}
    ...
    TkTextUndoInfo *redoInfo)	/* Store undo information, can be NULL. */
{
	...
	if (segPtr1) { segPtr1->protectionFlag = 1; }
    ...
    int32_t *pixelOffset)	/* Used to return offset. */
{
	...
	if (TkBTreeLinesTo(tree, NULL, linePtr, NULL) >
		TkBTreeLinesTo(tree, NULL, endLinePtr, NULL)) {
	    linePtr = endLinePtr;
	}
    }
    ...
    int setTagoff)
{
	...
	if (setTagoff) {
		...
		setTagoff = 1; /* but parent now has tagoff */
	    } else if (nchilds < nodePtr->numLines) {
		nodePtr->tagoffPtr = TagSetAdd(nodePtr->tagoffPtr, tagPtr);
		setTagoff = 1; /* propagate to parent */
	    }
	}
    ...
    unsigned reason)
{
    ...
    TkTextSegment *keptBranchPtr;
				/* Walked branch which still opens the current
				 * elided range, not reconnected yet. */
    ...
    prevBranchPtr = lastBranchPtr = newBranchPtr = keptBranchPtr = NULL;
	...
	if (!segPtr) {
		...
		/*
		 * In recompute mode (tagPtr is NULL) a line is unaffected only
		 * if the walk is currently outside of an elided range and the
		 * line carries no elision relevant tags: an untagged line
		 * inside an elided range is a transition which has to be
		 * processed (in normal mode the range extension guarantees
		 * that the boundary lines carry the toggled tag).
		 */
		while (linePtr != lastLinePtr
			...
			&& (tagPtr
			    ...
			    : (!actualElided && !TkTextTagSetIntersectsBits(
				    linePtr->tagonPtr, sharedTextPtr->elisionTags)))) {
	...
	if (segPtr->tagInfoPtr) {
		...
		if (shouldBeElided || !actualElided) {
		    ...
		    /*
		     * The kept link closes the elided range opened by the
		     * branch inserted in this pass: connect them now. If a
		     * further elided range follows within the walk, its
		     * switches would reset the link bookkeeping, and the
		    ...
		    /*
		     * The kept link closes the elided range opened by a KEPT
		     * branch whose pairing was broken (switches restored
		...
		if (shouldBeElided) {
		    ...
		    newBranchPtr = lastBranchPtr = keptBranchPtr = NULL;
		    somethingHasChanged = 1;
		}
	    }
    ...
    TkTextTag *tagPtr)
{
	...
	firstSegPtr->protectionFlag = 1;
	search.segPtr->protectionFlag = 1;
    ...
    int redraw)
{
	...
	const TkSharedText *sharedTextPtr = tagPtr->sharedTextPtr;
	int delegateRedraw = redraw && NodeTestAnySegment(nodePtr, tagPtr->index, add);
	TkTextIndex index1, index2;
    ...
    TkTextSegment **segPtr2)
{
	...
	if (tagPtr) {
		...
		return 0; /* whole range is already tagged/untagged */
	    }
	} else {
	    if (!TkBTreeFindNextTagged(indexPtr1, indexPtr2, NULL)) {
		return 0; /* whole range is already untagged */
	    }
	}
    }
	...
	if (segPtr->tagInfoPtr) {
		...
		if (undoToken) {
			...
			if (data->skip == 0
			    ...
			    /*
			     * The undo walk starts at the beginning of the
			     * line of the first recorded segment (this will
			     * become the start index of the token), so the
			     * skip of the first entry has to be anchored
			     * there: bytes accumulated over earlier lines of
			     * the range (start lines without a recordable
			     * segment: tags ending before the range, or tags
			     * excluded from undo) do not belong into it.
			     */
			    tagChangePtr->skip = (undoToken->changeListSize == 1)
				    ? (uint32_t) TkTextSegToIndex(segPtr) : data->skip;
    ...
    TkText *textPtr)
{
	...
	for ( ; linePtr != lastPtr; linePtr = linePtr->nextPtr) {
		...
		ClearTagsFromLine(sharedTextPtr, linePtr, NULL, NULL, NULL, NULL, data,
			discardSelection, 0, changedProc, textPtr);
	    } else if (data->firstSegPtr) {
		data->skip += linePtr->size;
	    }
	}
    ...
    TkTextTagChangedProc changedProc)	/* Trigger this callback when any tag will be added/removed. */
{
    ...
    if (!FindSplitPoints(sharedTextPtr, &startIndex, &endIndex, NULL, 0, &segPtr1, &segPtr2)) {
	return NULL;
    }
    ...
    unsigned tagIndex)
{
	...
	if (nodePtr != excludePtr && startLineNo < lineNumber + nodePtr->numLines) {
	    int testTagon = !LineTestIfToggleIsOpen(nodePtr->linePtr->prevPtr, tagIndex);
    ...
    unsigned tagIndex)
{
	...
	if (nodePtr != excludePtr && startLineNo >= lineNumber - nodePtr->numLines + 1) {
	    int testTagon = !LineTestIfToggleIsClosed(nodePtr->lastPtr->nextPtr, tagIndex);
    ...
    const TkTextIndex *stopIndex)
{
    ...
    if (!testTagon && NodeTestAllSegments(nodePtr, tagIndex, 1)) {
 ...
 *
 * Results:
 *      The return value is the language string / locale belonging to given segment.
 *
 * Side effects:
 *	None.
 *
    ...
    TkTextSegment *startMarker)
{
    ...
    /*
     * At second, search for line with a branch in current node. Note that
     * the first line has to be examined before giving up (the bail-out
     * tested the predecessor, so a branch on the first line was never
     * found when the search started two or more lines below it).
     */
	...
	if (linePtr == firstLinePtr || !linePtr->prevPtr) {
	    return startMarker;
	}
	linePtr = linePtr->prevPtr;
    }
    ...
    TkTextSegment *segPtr)	/* Segment to reuse. */
{
    ...
    segPtr->body.branch.nextPtr->body.link.prevPtr = segPtr;
    return 1;
}

/*
    ...
    TkTextSegment *segPtr)	/* Segment to reuse. */
{
    ...
    /* The counterpart may have been re-connected meanwhile (see DeleteRange). */
    segPtr->body.link.prevPtr->body.branch.nextPtr = segPtr;
    return 1;
}

/*
    ...
    const TkTextLine *linePtr)		/* Pointer to line in B-tree */
{
    ...
    unsigned numBranches = 0;
    unsigned numLinks = 0;
    int startsWithBranch = 0;
    int startsWithLink = 0;
    ...
    const TkTextLine *linePtr)	/* Pointer to line with sections. */
{
	...
	if (numSegs < MIN_TEXT_SEGS && sectionPtr->nextPtr) {
	    /*
	     * A short section (not the last one of the line) is allowed only
	     * if both of its boundaries are forced by the switch rules (a
	     * link can only head a section, a branch can only terminate one)
	     * or by the line start. The boundaries are tested independently,
	     * not as pairs: during the restore phase of an undo the switch
	     * chain may be transiently nested, which produces unmergeable
	     * short sections between two switches of the same direction.
	     */
	    if (sectionPtr->prevPtr
		    && sectionPtr->segPtr->typePtr != &tkTextLinkType
		    && sectionPtr->segPtr->prevPtr->typePtr != &tkTextBranchType) {
		Tcl_Panic("CheckSections: too few segments in section");
	    }
	    if (sectionPtr->nextPtr->segPtr->typePtr != &tkTextLinkType
