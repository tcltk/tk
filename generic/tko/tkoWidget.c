	...
	Tcl_Obj *const objv[])	/* Argument objects. */
{
	static const char *const myOptions[] = {
		"initfrom", "optionadd", "optiondel","optionget",
		"optionset","optionhide","optionshow",NULL
	};
	enum options {
		MY_INITFROM, MY_OPTIONADD, MY_OPTIONDEL, MY_OPTIONGET,
		MY_OPTIONSET, MY_OPTIONHIDE, MY_OPTIONSHOW
	};
	int index;
	...
	case MY_INITFROM:
		...
		if (objc == 3) {
			tmpPtr = Tcl_ObjPrintf("superclass %s", Tcl_GetString(objv[2]));
			...
			tmpPtr = Tcl_ObjPrintf("set ::tko::_option([self]) [::tko optionget %s]", Tcl_GetString(objv[2]));
			Tcl_IncrRefCount(tmpPtr);
			ret = Tcl_Eval(interp, Tcl_GetString(tmpPtr));
			Tcl_DecrRefCount(tmpPtr);
            if (ret != TCL_OK) {
		...
		return TCL_OK;
	/* Add or option definition and create method if given */
	case MY_OPTIONADD:
		if (objc != 8 && objc != 9) {
			Tcl_WrongNumArgs(interp, 2, objv, "::classname -option dbname dbclass value flags ?body?");
			return TCL_ERROR;
		}
		/* get current value or create new one */
		dictPtr = Tcl_ObjGetVar2(interp, TkoObj_tko__option, objv[2], TCL_GLOBAL_ONLY);
		if (dictPtr == NULL) {
			dictPtr = Tcl_NewObj();
		}
		else {
		...
		if (WidgetOptionAdd(interp, NULL, objv[3], objv[4], objv[5], objv[6], objv[7], NULL, 0) != TCL_OK) {
		...
		tmpPtr = Tcl_NewObj();
		Tcl_IncrRefCount(tmpPtr);
		Tcl_ListObjAppendElement(interp, tmpPtr, objv[4]);
		Tcl_ListObjAppendElement(interp, tmpPtr, objv[5]);
		Tcl_ListObjAppendElement(interp, tmpPtr, objv[6]);
		Tcl_ListObjAppendElement(interp, tmpPtr, objv[7]);
		if (Tcl_DictObjPut(interp, dictPtr, objv[3], tmpPtr) != TCL_OK) {
			Tcl_DecrRefCount(tmpPtr);
			Tcl_DecrRefCount(dictPtr);
			return TCL_ERROR;
		}
		Tcl_DecrRefCount(tmpPtr);
		tmpPtr = Tcl_ObjSetVar2(interp, TkoObj_tko__option, objv[2], dictPtr, TCL_GLOBAL_ONLY);
		Tcl_DecrRefCount(dictPtr);
		if (tmpPtr == NULL) {
			return TCL_ERROR;
		}
		...
		return TCL_OK;
	/* Delete option definitions and return new state */
	case MY_OPTIONDEL:
		if (objc < 3) {
		...
		if (objc == 3) {
			tmpPtr = Tcl_ObjSetVar2(interp, TkoObj_tko__option, objv[2],TkoObj_empty,TCL_GLOBAL_ONLY);
			if (tmpPtr == NULL) {
				return TCL_ERROR;
			}
			Tcl_SetObjResult(interp, tmpPtr);
		...
		Tcl_SetObjResult(interp, tmpPtr);
		return TCL_OK;
	/* Return all or selected option definitions */
	case MY_OPTIONGET:
		if (objc < 3) {
			Tcl_WrongNumArgs(interp, 2, objv, "::classname ?-option? ..");
			return TCL_ERROR;
		...
		Tcl_DecrRefCount(listPtr);
		return TCL_OK;
	/* Add or replace option definitions and return new state */
	case MY_OPTIONSET:
		if (objc < 3 || objc % 2 != 1) {
		...
		for (i = 3; i < objc; i = i + 2) {
			...
			if (Tcl_ListObjGetElements(interp, objv[i+1], &argObjc, &argObjv) != TCL_OK
				|| argObjc != 4) {
				Tcl_SetObjResult(interp, Tcl_ObjPrintf("wrong definition: %s {%s}",objv[i], objv[i+1]));
				Tcl_DecrRefCount(dictPtr);
				return TCL_ERROR;
			}
			if (WidgetOptionAdd(interp,NULL,objv[i],argObjv[0],argObjv[1],argObjv[2],argObjv[3],NULL,0)!=TCL_OK) {
				Tcl_DecrRefCount(dictPtr);
				return TCL_ERROR;
			}
			if (Tcl_DictObjPut(interp, dictPtr, objv[i], objv[i + 1]) != TCL_OK) {
				Tcl_DecrRefCount(dictPtr);
				return TCL_ERROR;
			}
		}
		...
		Tcl_SetObjResult(interp, tmpPtr);
		return TCL_OK;
	/* Hide given options or return all hide'able options */
	case MY_OPTIONHIDE:
		if (objc < 3) {
			Tcl_WrongNumArgs(interp, 2, objv, "::classname ?-option? ..");
			return TCL_ERROR;
		...
		return TCL_OK;
	/* Show given options or return all hidden options */
	case MY_OPTIONSHOW:
		if (objc < 3) {
			Tcl_WrongNumArgs(interp, 2, objv, "::classname ?-option? ..");
			return TCL_ERROR;
	...
	Tcl_Obj * const objv[])
{
	...
	switch ((enum options) index) {
	case MY_OPTIONADD:
		if ((objc - skip) != 6 && (objc-skip) != 7) {
			Tcl_WrongNumArgs(interp, skip + 1, objv,
				"-option dbname dbclass value flags ?body?");
			return TCL_ERROR;
		}
		/* First add the method if given. */
		if ((objc - skip) == 7) {
		...
		if (WidgetOptionAdd(interp, widget, objv[skip + 1], objv[skip + 2],
			objv[skip + 3], objv[skip + 4], objv[skip + 5], NULL, 0) != TCL_OK) {
			return TCL_ERROR;
		}
		return TCL_OK;
    ...
    Tko_WidgetOptionDefine * options)
{
	...
	static Tcl_MethodType widgetMethods[] = {
		...
		{ TCL_OO_METHOD_VERSION_CURRENT, "_tko",
		WidgetMethod_tko, NULL, NULL },
	}; /* List of all used public and private methods. */
    ...
    if(methods) {
		...
		/* our own methods */
        Tcl_NewMethod(interp, clazz, TkoObj_cget, 1, &widgetMethods[0], NULL);
        Tcl_NewMethod(interp, clazz, TkoObj_configure, 1, &widgetMethods[1], NULL);
        Tcl_NewMethod(interp, clazz, TkoObj__tko_configure, 0, &widgetMethods[2], NULL);
        Tcl_NewMethod(interp, clazz, TkoObj__tko, 0, &widgetMethods[3], NULL);
		/* public */
    ...
    if(options) {
        ...
        for(i = 0;; i++) {
			...
			if (options[i].method != NULL) {
				...
				Tcl_NewMethod(interp, clazz, optionPtr, 0, methodPtr, NULL);
			}
				...
				methodPtr->deleteProc = NULL;
				methodPtr->cloneProc = NULL;
				Tcl_NewMethod(interp, clazz, optionPtr, 0, methodPtr,
	...
	Tcl_Obj *arglist) /* -value option .. list, used options will be removed */
{
	...
	/* Create and initialize internal widget strucure. */
	widget->interp = interp;
	widget->object = object;
	widget->tkWin = NULL;
	widget->display = NULL;
	widget->widgetCmd = NULL;
	...
	/* Convert argumment list in dictionary */
	...
	if (isToplevel) {
		...
		tkWin = Tk_CreateWindowFromPath(interp, Tk_MainWindow(interp), &ch[2],
	...
	if (isToplevel) {
		ret = WidgetOptionAdd(interp, widget, optionObjv[0], argObjv[0],
			argObjv[1], argObjv[2], argObjv[3], screen, 1);
    ...
    /*
     * Add options. "-class" option should be first option now.
     */
	for(; i < optionObjc; i=i+2) {
        if (Tcl_ListObjGetElements(interp, optionObjv[i+1], &argObjc, &argObjv) !=TCL_OK
            ...
            Tcl_SetObjResult(interp, Tcl_ObjPrintf("wrong option def: %s {%s}",
                    Tcl_GetString(optionObjv[i]),optionObjv[i+1]));
            goto error;
        }
		/* Option "-class" should be first option.
        ...
        if(WidgetOptionAdd(interp, widget, optionObjv[i], argObjv[0],
                argObjv[1], argObjv[2], argObjv[3], value, 1) != TCL_OK) {
            goto error;
        }
    }
    if(argSize) {
...
{              /* Information about event. */
	...
	case VirtualEvent:
		if (widget->tkWin) {
			if (!strcmp("OptiondbChanged", ((XVirtualEvent *)(eventPtr))->name)) {
	...
	Tko_Widget *widget)
{
    ...
    while (entryPtr != NULL) {
		...
		if (optionPtr->defvalue) continue;/* default value was given */
		  /*
...
WidgetMethod_(
	...
	Tcl_Obj * const objv[])
{
	Tko_WidgetOptionDefine *define = (Tko_WidgetOptionDefine *)clientData;
	Tcl_Object object;
	Tko_Widget *widget;
	Tcl_Obj *value;
	char *address = NULL;
	...
	if (objc != 2 || (object = Tcl_ObjectContextObject(context)) == NULL
		|| (widget =
		(Tko_Widget *)Tcl_ObjectGetMetadata(object, &tkoWidgetMeta)) == NULL
		|| (value =
			Tcl_ObjGetVar2(interp, widget->optionsArray, objv[1],
