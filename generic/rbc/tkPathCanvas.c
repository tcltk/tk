...
static const char** GetStringsFromObjs(int objc, Tcl_Obj *const *objv);
...
static void     TagSearchExprInit(TkPathTagSearchExpr **exprPtrPtr);
static void     TagSearchExprDestroy(TkPathTagSearchExpr *expr);
static void     TagSearchDestroy(TagSearch *searchPtr);
static int      TagSearchScan(TkPathCanvas *canvasPtr, Tcl_Obj *tag,
    TagSearch **searchPtrPtr);
static int      TagSearchScanExpr(Tcl_Interp *interp, TagSearch *searchPtr,
    TkPathTagSearchExpr *expr);
static int      TagSearchEvalExpr(TkPathTagSearchExpr *expr,
       Tk_PathItem *itemPtr);
static Tk_PathItem  *TagSearchFirst(TagSearch *searchPtr);
static Tk_PathItem  *TagSearchNext(TagSearch *searchPtr);
static int      PathConstructor(ClientData clientData, Tcl_Interp *interp,
    Tcl_ObjectContext objectContext, int objc, Tcl_Obj *const objv[]);
static int      PathMethod(ClientData clientData, Tcl_Interp *interp,
    Tcl_ObjectContext objectContext, int objc, Tcl_Obj *const objv[]);
static void     PathMetaDelete(ClientData clientData);
...
static Tcl_MethodType pathMethods[] = {
    ...
    {TCL_OO_METHOD_VERSION_CURRENT,"cget"      , PathMethod , NULL , NULL },
    {TCL_OO_METHOD_VERSION_CURRENT,"children" , PathMethod , NULL , NULL },
    {TCL_OO_METHOD_VERSION_CURRENT,"cmove" , PathMethod , NULL , NULL },
    {TCL_OO_METHOD_VERSION_CURRENT,"configure" , PathMethod , NULL , NULL },
 ...
 */
int
Tk_PathInit(Tcl_Interp *interp)        /* Tcl interpreter. */
{
    Tcl_Class clazz;
    Tcl_Object object;
    Tcl_Obj *objPtr;
	...
	static const char *initScript =
		...
		" self method unknown {args} {uplevel #0 ::path create $args; return [lindex $args 0]}\n"
    ...
    if (Tcl_CreateNamespace(interp, TK_PATHCMD_CLASS, NULL, NULL) == NULL) {
        return TCL_ERROR;
    }
    ...
    /* Get class object */
    objPtr = Tcl_NewStringObj(TK_PATHCMD_CLASS, -1);
    ...
    /* Add methods */
    Tcl_ClassSetConstructor(interp, clazz, Tcl_NewMethod(interp, clazz, NULL, 1, &pathMethods[0], NULL));
    for (i = 1; pathMethods[i].name != NULL; i++) {
        Tcl_NewMethod(interp, clazz, Tcl_NewStringObj(pathMethods[i].name, -1), 1, &pathMethods[i], NULL);
    }
    ...
    Tcl_Obj *const objv[])
{
    ...
    canvasPtr->tkwin = tkWin;
    canvasPtr->display = Tk_Display(tkWin);
    canvasPtr->interp = interp;
	canvasPtr->widgetCmd = Tcl_GetObjectCommand(object);
    ...
    Tcl_ObjectSetMetadata(object, &pathMeta, (ClientData)canvasPtr);

    if (Tk_InitOptions(interp, (char *)canvasPtr, optionTable, canvasPtr->tkwin) != TCL_OK
        || ConfigureCanvas(interp, canvasPtr, objc-3, objv+3, 0) != TCL_OK) {
     ...
     */
    ItemCreate(interp, canvasPtr, &tkPathTypeGroup, 1, &rootItemPtr, 0, NULL);
...
static int PathMethod(
    ...
    Tcl_Obj *const objv[])
{
    return CanvasWidgetCmd(Tcl_ObjectGetMetadata(Tcl_ObjectContextObject(objectContext), &pathMeta), interp, objc, objv);
}

static void
PathMetaDelete(ClientData clientData) {
    register TkPathCanvas *canvasPtr = clientData;

    /* Destroy window will calling DestroyCanvas() also. */
    if (canvasPtr && (!(canvasPtr->flags & CANVAS_DELETED))) {
        Tk_DestroyWindow(canvasPtr->tkwin);
    }
}

    ...
    Tcl_Obj *const objv[])    /* Argument objects. */
{
    ...
    case CANV_GRADIENT: {
    result = TkPathCanvasGradientObjCmd(interp, canvasPtr, objc, objv);
    break;
    }
    case CANV_ICURSOR: {
    int index;
    ...
    case CANV_STYLE: {
    result = TkPathCanvasStyleObjCmd(interp, canvasPtr, objc, objv);
    break;
    }
    case CANV_TYPE: {
    if (objc != 3) {
    ...
    char *memPtr)        /* Info about canvas widget. */
{
    ...
    /* @@@ TODO: tkwin = NULL! */
    TkPathStylesFree(canvasPtr->tkwin, &canvasPtr->styleTable);
    Tcl_DeleteHashTable(&canvasPtr->styleTable);

    TkPathCanvasGradientsFree(canvasPtr);
...
void
TkPathCanvasSetParentToRoot(Tk_PathItem *itemPtr)
{
...
void
TkPathGroupItemConfigured(Tk_PathCanvas canvas, Tk_PathItem *itemPtr, int mask)
{
    Tk_PathItem *walkPtr;
...
void
TkPathCanvasScaleGroup(Tk_PathCanvas canvas, Tk_PathItem *itemPtr, int compensate,
