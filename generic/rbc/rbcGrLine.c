...
static Tk_ConfigSpec linePenConfigSpecs[] = {
    ...
    {TK_CONFIG_CUSTOM, "-fill", "fill", "Fill",
     ...
     TK_CONFIG_NULL_OK | TK_CONFIG_COLOR_ONLY | RBC_ALL_PENS, &rbcColorOption},
    {TK_CONFIG_CUSTOM, "-fill", "fill", "Fill",
     DEF_PEN_FILL_MONO, Tk_Offset(LinePen, symbol.fillColor),
     TK_CONFIG_NULL_OK | TK_CONFIG_MONO_ONLY | RBC_ALL_PENS, &rbcColorOption},
    {TK_CONFIG_CUSTOM, "-linewidth", "lineWidth", "LineWidth",
    ...
    {TK_CONFIG_STRING, "-type", (char *)NULL, (char *)NULL,
     DEF_PEN_TYPE, Tk_Offset(RbcPen, typeId), RBC_ALL_PENS | TK_CONFIG_NULL_OK},
    {TK_CONFIG_ANCHOR, "-valueanchor", "valueAnchor", "ValueAnchor",
     DEF_PEN_VALUE_ANCHOR, Tk_Offset(LinePen, valueStyle.anchor), RBC_ALL_PENS},
    {TK_CONFIG_COLOR, "-valuecolor", "valueColor", "ValueColor",
     DEF_PEN_VALUE_COLOR, Tk_Offset(LinePen, valueStyle.color), RBC_ALL_PENS},
    {TK_CONFIG_FONT, "-valuefont", "valueFont", "ValueFont",
     DEF_PEN_VALUE_FONT, Tk_Offset(LinePen, valueStyle.font), RBC_ALL_PENS},
    {TK_CONFIG_STRING, "-valueformat", "valueFormat", "ValueFormat",
     DEF_PEN_VALUE_FORMAT, Tk_Offset(LinePen, valueFormat),
     RBC_ALL_PENS | TK_CONFIG_NULL_OK},
    {TK_CONFIG_DOUBLE, "-valuerotate", "valueRotate", "ValueRotate",
     DEF_PEN_VALUE_ROTATE, Tk_Offset(LinePen, valueStyle.theta), RBC_ALL_PENS},
    {TK_CONFIG_CUSTOM, "-valueshadow", "valueShadow", "ValueShadow",
     DEF_PEN_VALUE_SHADOW, Tk_Offset(LinePen, valueStyle.shadow),
     RBC_ALL_PENS, &rbcShadowOption},
    {TK_CONFIG_END, NULL, NULL, NULL, NULL, 0, 0}
};
typedef double (DistanceProc) (int x, int y, RbcPoint2D *p,
        RbcPoint2D *q, RbcPoint2D *t);
...
static const char *NameOfSymbol (Symbol *symbolPtr);
static const char *NameOfSmooth (Smoothing value);
static const char *NameOfPenDir (int penDir);
static void ClearPalette (RbcChain *palette);
static void InitPen (LinePen *penPtr);
static int ScaleSymbol (RbcElement *elemPtr, int normalSize);
static void GetScreenPoints (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void ReducePoints (MapInfo *mapPtr, double tolerance);
static void GenerateSteps (MapInfo *mapPtr);
static void GenerateSpline (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void GenerateParametricSpline (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void MapSymbols (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void MapActiveSymbols (RbcGraph *graphPtr, Line *linePtr);
static void MapStrip (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void MergePens (Line *linePtr, RbcPenStyle **dataToStyle);
static int OutCode (RbcExtents2D *extsPtr, RbcPoint2D *p);
static int ClipSegment (RbcExtents2D *extsPtr, register int code1, register int code2, register RbcPoint2D *p, register RbcPoint2D *q);
static void SaveTrace (Line *linePtr, int start, int length, MapInfo *mapPtr);
static void FreeTraces (Line *linePtr);
static void MapTraces (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void MapFillArea (RbcGraph *graphPtr, Line *linePtr, MapInfo *mapPtr);
static void ResetLine (Line *linePtr);
static int ClosestTrace (RbcGraph *graphPtr, Line *linePtr, RbcClosestSearch *searchPtr, DistanceProc *distProc);
static int ClosestStrip (RbcGraph *graphPtr, Line *linePtr, RbcClosestSearch *searchPtr, DistanceProc *distProc);
static void ClosestPoint (Line *linePtr, RbcClosestSearch *searchPtr);
static void DrawCircles (Display *display, Drawable drawable, Line *linePtr, LinePen *penPtr, int nSymbolPts, RbcPoint2D *symbolPts, int radius);
static void DrawSquares (Display *display, Drawable drawable, Line *linePtr, LinePen *penPtr, int nSymbolPts, register RbcPoint2D *symbolPts, int r);
static void DrawSymbols (RbcGraph *graphPtr, Drawable drawable, Line *linePtr, LinePen *penPtr, int size, int nSymbolPts, RbcPoint2D *symbolPts);
static void DrawTraces (RbcGraph *graphPtr, Drawable drawable, Line *linePtr, LinePen *penPtr);
static void DrawValues (RbcGraph *graphPtr, Drawable drawable, Line *linePtr, LinePen *penPtr, int nSymbolPts, RbcPoint2D *symbolPts, int *pointToData);
static void GetSymbolPostScriptInfo (RbcGraph *graphPtr, RbcPsToken *psToken, LinePen *penPtr, int size);
static void SymbolsToPostScript (RbcGraph *graphPtr, RbcPsToken *psToken, LinePen *penPtr, int size, int nSymbolPts, RbcPoint2D *symbolPts);
static void SetLineAttributes (RbcPsToken *psToken, LinePen *penPtr);
static void TracesToPostScript (RbcPsToken *psToken, Line *linePtr, LinePen *penPtr);
static void ValuesToPostScript (RbcPsToken *psToken, Line *linePtr, LinePen *penPtr, int nSymbolPts, RbcPoint2D *symbolPts, int *pointToData);
/*
 *----------------------------------------------------------------------
 *
 * Round --
    ...
    RbcPen *penPtr)
{
    ...
    XGCValues gcValues;
    XColor *colorPtr;
    RbcResetTextStyle(graphPtr->tkwin, &(lpPtr->valueStyle));
    /*
    ...
    if (lpPtr->traceGC != NULL) {
        RbcFreePrivateGC(graphPtr->display, lpPtr->traceGC);
    }
    if (RbcLineIsDashed(lpPtr->traceDashes)) {
        lpPtr->traceDashes.offset = lpPtr->traceDashes.values[0] / 2;
        RbcSetDashes(graphPtr->display, newGC, &(lpPtr->traceDashes));
    }
    lpPtr->traceGC = newGC;
    gcMask = (GCLineWidth | GCForeground);
...
{
    LinePen *lpPtr = (LinePen *)penPtr;
    RbcFreeTextStyle(graphPtr->display, &(lpPtr->valueStyle));
    if (lpPtr->symbol.outlineGC != NULL) {
        Tk_FreeGC(graphPtr->display, lpPtr->symbol.outlineGC);
    }
    if (lpPtr->symbol.fillGC != NULL) {
    ...
    if (graphPtr->inverted) {
        for (i = 0; i < n; i++) {
            if ((!TclIsInfinite(x[i])) && (!TclIsInfinite(y[i]))) {
                screenPts[count].x = RbcHMap(graphPtr, linePtr->axes.y, y[i]);
                screenPts[count].y = RbcVMap(graphPtr, linePtr->axes.x, x[i]);
                indices[count] = i;
                count++;
            }
        }
        ...
        for (i = 0; i < n; i++) {
            if ((!TclIsInfinite(x[i])) && (!TclIsInfinite(y[i]))) {
                screenPts[count].x = RbcHMap(graphPtr, linePtr->axes.x, x[i]);
                screenPts[count].y = RbcVMap(graphPtr, linePtr->axes.y, y[i]);
                indices[count] = i;
                count++;
            }
        }
    ...
    Line *linePtr)
{
    ...
    RbcGraphExtents(graphPtr, &exts);
    activePts = (RbcPoint2D *)ckalloc(sizeof(RbcPoint2D) * linePtr->nActiveIndices);
    assert(activePts);
    ...
    for (i = 0; i < linePtr->nActiveIndices; i++) {
        ...
        activePts[count] = RbcMap2D(graphPtr, x, y, &(linePtr->axes));
    ...
    MapInfo *mapPtr)
{
     ...
     */
    segPtr = strips = (RbcSegment2D *)ckalloc(mapPtr->nScreenPts * sizeof(RbcSegment2D));
    assert(strips);
    RbcGraphExtents(graphPtr, &exts);
    ...
    RbcPenStyle **dataToStyle)
{
    ...
    if (linePtr->nStrips > 0) {
        ...
        int dataIndex;
        strips = (RbcSegment2D *)ckalloc(linePtr->nStrips * sizeof(RbcSegment2D));
    ...
    if (linePtr->nSymbolPts > 0) {
        ...
        int dataIndex;
        symbolPts = (RbcPoint2D *)ckalloc(linePtr->nSymbolPts * sizeof(RbcPoint2D));
    ...
    if (linePtr->xErrorBarCnt > 0) {
        ...
        int dataIndex;
        xErrorBars = (RbcSegment2D *)ckalloc(linePtr->xErrorBarCnt * sizeof(RbcSegment2D));
    ...
    if (linePtr->yErrorBarCnt > 0) {
        ...
        int dataIndex;
        errorBars = (RbcSegment2D *)ckalloc(linePtr->yErrorBarCnt * sizeof(RbcSegment2D));
...
MapLine(
    ...
    RbcElement *elemPtr) /* Element component record */
{
    ...
    if (((linePtr->yHigh.nValues > 0) && (linePtr->yLow.nValues > 0)) ||
        ...
        RbcMapErrorBars(graphPtr, (RbcElement *)linePtr, dataToStyle);
    }
...
DistanceToLine(
    ...
    RbcPoint2D *t) /* (out) Point on line segment. */
{
    ...
    DistanceProc *distProc)
{
    ...
    if (minDist < searchPtr->dist) {
        ...
        searchPtr->index = i;
        searchPtr->point = RbcInvMap2D(graphPtr, closest.x, closest.y, &(linePtr->axes));
        return TRUE;
    }
    return FALSE;
}
...
ClosestPoint(
    ...
    RbcClosestSearch *searchPtr) /* Assorted information related to searching
    ...
    RbcElement *elemPtr)
{
    ...
    GC newGC;
    RbcChainLink *linkPtr;
    if (ConfigurePen(graphPtr, (RbcPen *)&(linePtr->builtinPen)) != TCL_OK) {
        return TCL_ERROR;
    }
    /*
    ...
    if (RbcConfigModified(linePtr->configSpecs, "-scalesymbols",
                           (char *)NULL)) {
...
DrawSymbols(
    ...
    RbcPoint2D *symbolPts) /* Array of x,y coordinates for line */
{
...
#define COS30		0.86602540378443871
        ...
        case SYMBOL_BITMAP: {
                ...
                if (penPtr->symbol.mask != None) {
                    mask = RbcScaleBitmap(graphPtr->tkwin, penPtr->symbol.mask, width, height, bmWidth, bmHeight);
                    XSetClipMask(graphPtr->display, penPtr->symbol.outlineGC,
                                 mask);
                }
                bitmap = RbcScaleBitmap(graphPtr->tkwin, penPtr->symbol.bitmap, width, height, bmWidth, bmHeight);
                if (penPtr->symbol.fillGC == NULL) {
                    XSetClipMask(graphPtr->display, penPtr->symbol.outlineGC,
                                 bitmap);
                }
    ...
    LinePen *penPtr)
{
    ...
    if (penPtr->traceGC->line_width > 1) {
        ...
        nPoints = RbcMaxRequestSize(graphPtr->display, sizeof(POINT)) - 1;
    }
    ...
    LinePen *penPtr)
{
    ...
    register XPoint *p;
    register int count;
    nPoints = RbcMaxRequestSize(graphPtr->display, sizeof(XPoint)) - 1;
...
DrawActiveLine(
    RbcGraph *graphPtr, /* Graph widget record */
    Drawable drawable, /* Pixmap or window to draw into */
    RbcElement *elemPtr) /* Element to be drawn */
{
    Line *linePtr = (Line *)elemPtr;
    ...
    if (linePtr->nActiveIndices > 0) {
        ...
        if (penPtr->traceWidth > 0) {
            if (linePtr->nStrips > 0) {
                RbcDraw2DSegments(graphPtr->display, drawable,
    ...
    RbcElement *elemPtr) /* Element to be drawn */
{
    ...
    if (linePtr->fillPts != NULL) {
        ...
        if (linePtr->fillTile != NULL) {
            RbcSetTileOrigin(graphPtr->tkwin, linePtr->fillTile, 0, 0);
            RbcTilePolygon(graphPtr->tkwin, drawable, linePtr->fillTile,
    ...
    switch (penPtr->symbol.type) {
        ...
        case SYMBOL_BITMAP: {
                ...
                if ((penPtr->symbol.mask != None) && (fillColor != NULL)) {
                    ...
                    RbcBitmapToPostScript(psToken, graphPtr->display,
    ...
    RbcElement *elemPtr)
{
    ...
    if (linePtr->fillPts != NULL) {
        ...
        if (linePtr->fillTile != NULL) {
            ...
            RbcStippleToPostScript(psToken, graphPtr->display,
                                    linePtr->fillStipple);
        } else {
            RbcAppendToPostScript(psToken, "Fill\n", (char *)NULL);
        }
    ...
    RbcElement *elemPtr)
{
    ...
    if (linePtr->normalPenPtr != &(linePtr->builtinPen)) {
        RbcFreePen(graphPtr, (RbcPen *)linePtr->normalPenPtr);
    }
    DestroyPen(graphPtr, (RbcPen *)&(linePtr->builtinPen));
    if (linePtr->activePenPtr != NULL) {
        RbcFreePen(graphPtr, (RbcPen *)linePtr->activePenPtr);
    }
    ...
    if (linePtr->palette != NULL) {
        RbcFreePalette(graphPtr, linePtr->palette);
