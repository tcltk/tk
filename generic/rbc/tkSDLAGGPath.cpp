...
static void
textLineU(Agg2D *agg2d, double &x, double &y, const unsigned **string)
{
	       ...
	       int fillOverStroke, char *utf8, void *custom)
{
    ...
    context->agg2d->font(tconf->fontName, textStylePtr->fontSize,
			 ...
			 (const char *) XGetFTStream(tconf->fontFile,
						     tconf->fontFileSize),
			 0);
	...
	context->agg2d->lineColor(XC_R(style->strokeColor),
				  ...
				  XC_B(style->strokeColor),
				  (unsigned) (opacity * 0xFF));
			  ...
			  double fillOpacity, TMatrix *mPtr)
{
	...
	for (int i = 0; i < nstops; i++) {
	    ...
	    Agg2D::Color color(XC_R(stop->color),
			       XC_G(stop->color),
			       XC_B(stop->color),
			       (unsigned) (opacity * fillOpacity * 0xFF));
	    colors[i] = color;
	}
