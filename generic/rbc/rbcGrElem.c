    ...
    RbcPenStyle * stylePtr)
{
    ...
    if (RbcGetPen(graphPtr, elemArr[0], type, &penPtr) != TCL_OK) {
        ckfree((char *) elemArr);
        return TCL_ERROR;
    }
    if (nElem == 3) {
    ...
    double **arrayPtr)
{
    ...
    if (nElem > 0) {
        ...
        if (array == NULL) {
            Tcl_AppendResult(interp, "can't allocate new vector",
                (char *) NULL);
            goto badList;
        }
        valuePtr = array;
    ...
    RbcChain * palette)
{
    ...
    if (linkPtr != NULL) {
        ...
        for (linkPtr = RbcChainNextLink(linkPtr); linkPtr != NULL;
            ...
            RbcFreePen(graphPtr, stylePtr->penPtr);
        ...
        for (i = 0; i < n; i++) {
            ...
            if ((!TclIsInfinite(x)) && (!TclIsInfinite(y))) {
                ...
                if ((!TclIsInfinite(high)) && (!TclIsInfinite(low))) {
                    ...
                    p = RbcMap2D(graphPtr, high, y, &elemPtr->axes);
                    q = RbcMap2D(graphPtr, low, y, &elemPtr->axes);
                    segPtr->p = p;
                    segPtr->q = q;
        ...
        for (i = 0; i < n; i++) {
            ...
            if ((!TclIsInfinite(x)) && (!TclIsInfinite(y))) {
                ...
                if ((!TclIsInfinite(high)) && (!TclIsInfinite(low))) {
                    ...
                    p = RbcMap2D(graphPtr, x, high, &elemPtr->axes);
                    q = RbcMap2D(graphPtr, x, low, &elemPtr->axes);
                    segPtr->p = p;
                    segPtr->q = q;
                    if (RbcLineRectClip(&exts, &segPtr->p, &segPtr->q)) {
                        segPtr++;
    ...
    RbcElement * elemPtr)
{
    ...
    RbcDeleteBindings(graphPtr->bindTable, elemPtr);
    RbcLegendRemoveElement(graphPtr->legend, elemPtr);
    ...
    for (linkPtr = RbcChainFirstLink(graphPtr->elements.displayList);
        ...
        if (elemPtr == RbcChainGetValue(linkPtr)) {
            RbcChainDeleteLink(graphPtr->elements.displayList, linkPtr);
            if (!elemPtr->hidden) {
    ...
    RbcUid classUid)
{
    ...
    if (classUid == rbcBarElementUid) {
        elemPtr = RbcBarElement(graphPtr, argv[3], classUid);
    } else {
        /* Stripcharts are line graphs with some options enabled. */
        elemPtr = RbcLineElement(graphPtr, argv[3], classUid);
    }
    elemPtr->hashPtr = hPtr;
    Tcl_SetHashValue(hPtr, elemPtr);

    if (RbcConfigureWidgetComponent(interp, graphPtr->tkwin, elemPtr->name,
    ...
    RbcChainPrepend(graphPtr->elements.displayList, elemPtr);
...
{                               /* Tcl list of element names */
    ...
    RbcChainReset(graphPtr->elements.displayList);
    ...
    for (i = 0; i < nNames; i++) {
        if (NameToElement(graphPtr, nameArr[i], &elemPtr) == TCL_OK) {
            ...
            RbcChainAppend(graphPtr->elements.displayList, elemPtr);
        }
    }
    ckfree((char *) nameArr);
    graphPtr->flags |= RBC_RESET_WORLD;
    ...
    RbcGraph * graphPtr)
{
    ...
    RbcChainDestroy(graphPtr->elements.displayList);
}
    ...
    RbcGraph * graphPtr)
{
    ...
    for (linkPtr = RbcChainFirstLink(graphPtr->elements.displayList);
        ...
        if ((graphPtr->flags & RBC_MAP_ALL) || (elemPtr->flags & RBC_MAP_ITEM)) {
...
static int
ActivateOp(
    RbcGraph * graphPtr,        /* Graph widget */
    ...
    const char *argv[])
{
    ...
    graphPtr->flags |= (RBC_REDRAW_BACKING_STORE | RBC_DRAW_MARGINS);
    RbcEventuallyRedrawGraph(graphPtr);
    return TCL_OK;
}
    ...
    const char *argv[])
{
    ...
    if ((argv[3][0] == 'c') && (strcmp(argv[3], "current") == 0)) {
        elemPtr = (RbcElement *) RbcGetCurrentItem(graphPtr->bindTable);
