...
static int        GetSubpathMaxNumSegments(TkPathAtom *atomPtr);
...
static void
NormalizePathRect(TkPathRect *r)
{
    double min, max;
    ...
    int width, int height)  /* must be redisplayed (not used). */
{
    ...
    if (pathPtr->pathLen > 2) {
         ...
         */
        TkPathDisplayArrow(canvas, &pathPtr->startarrow, &style, &m, &itemPtr->bbox);
        TkPathDisplayArrow(canvas, &pathPtr->endarrow, &style, &m, &itemPtr->bbox);
...
static int
GetSubpathMaxNumSegments(TkPathAtom *atomPtr)
{
    int            num;
    int         maxNumSegments;
