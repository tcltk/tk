...
CgetOp(
    ...
    const char *argv[])
{
    RbcPostScript  *psPtr = (RbcPostScript *) graphPtr->postscript;
    ...
    RbcPostScript * psPtr)
{
    ...
    graphPtr->flags |= RBC_LAYOUT_NEEDED | RBC_MAP_WORLD;
    ...
    RbcPsToken * psToken)
{
    ...
    image =
        RbcDrawableToColorImage(graphPtr->tkwin, drawable, 0, 0,
    ...
    RbcPsToken * psToken)
{
    ...
    if (psPtr->decorations) {
        RbcBackgroundToPostScript(psToken, Tk_3DBorderColor(graphPtr->border));
    } else {
    ...
    if (RbcLegendSite(graphPtr->legend) & RBC_LEGEND_IN_MARGIN) {
         ...
         */
        RbcLegendToPostScript(graphPtr->legend, psToken);
    }
    if (graphPtr->title != NULL) {
        RbcTextToPostScript(psToken, graphPtr->title,
            &graphPtr->titleTextStyle, (double) graphPtr->titleX,
            (double) graphPtr->titleY);
    }
    RbcAxesToPostScript(graphPtr, psToken);
}
    ...
    RbcPsToken * psToken)
{
    ...
    RbcFontToPostScript(psToken, graphPtr->titleTextStyle.font);
    RbcRegionToPostScript(psToken, (double) x, (double) y, width, height);
    if (graphPtr->postscript->decorations) {
        RbcBackgroundToPostScript(psToken, graphPtr->plotBg);
    } else {
        RbcClearBackgroundToPostScript(psToken);
    }
    ...
    if (!graphPtr->gridPtr->hidden) {
        RbcGridToPostScript(graphPtr, psToken);
    }
    RbcMarkersToPostScript(graphPtr, psToken, TRUE);
    ...
    RbcAxisLimitsToPostScript(graphPtr, psToken);
    RbcElementsToPostScript(graphPtr, psToken);
    if ((RbcLegendSite(graphPtr->legend) & RBC_LEGEND_IN_PLOT) &&
        (RbcLegendIsRaised(graphPtr->legend))) {
        /* Print legend above elements (but not markers) */
        RbcLegendToPostScript(graphPtr->legend, psToken);
    }
    RbcMarkersToPostScript(graphPtr, psToken, FALSE);
    RbcActiveElementsToPostScript(graphPtr, psToken);
    RbcAppendToPostScript(psToken, "\n",
        "% Unset clipping\n", "grestore\n\n", (char *) NULL);
...
{                               /* Option vector */
    ...
    psToken = RbcGetPsToken(graphPtr->interp, graphPtr->tkwin);
