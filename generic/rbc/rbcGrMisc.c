...
typedef struct {
    ...
    int             refCount;   /* Number of active uses of this color
                                 ...
                                 * call to RbcGet3DBorder).  If this
    ...
    struct Rbc3DBorderStruct *nextPtr;
} Border       ,
               *Rbc3DBorder;
...
RbcDraw3DRectangle(
    Tk_Window tkwin,            /* Window for which border was allocated. */
    Drawable drawable,          /* X window or pixmap in which to draw. */
    Rbc3DBorder * borderPtr,    /* Border to draw. */
    int x,                      /* Outside area of rectangular region. */
    int y,                      /* Outside area of rectangular region. */
    int width,                  /* Outside area of rectangular region. */
    int height,                 /* Outside area of rectangular region. */
    ...
    Border * borderPtr)
{
    ...
    if (borderPtr->darkGC != NULL) {
        RbcFreePrivateGC(display, borderPtr->darkGC);
    }
    if (borderPtr->lightGC != NULL) {
        RbcFreePrivateGC(tkwin, borderPtr->lightGC);
    }
    if (borderPtr->bgGC != NULL) {
        RbcFreePrivateGC(tkwin, borderPtr->bgGC);
    }
...
RbcGet3DBorder(
    ...
    char *borderName)
{
    Rbc3DBorder    *borderPtr, *lastBorderPtr;
    Tcl_HashEntry  *hPtr;
    RbcTile         tile;
    XColor         *bgColorPtr;
    char          **argv;
    ...
    if ((argc == 2) && (RbcGetTile(interp, tkwin, argv[0], &tile) == TCL_OK)) {
        colorName = argv[1];
    }
    ...
    /* Create a new border */
    borderPtr = RbcCalloc(1, sizeof(Rbc3DBorder));
    assert(borderPtr);
    ...
    borderPtr->tile = tile;
    borderPtr->darkGC = RbcGetPrivateGC(tkwin, 0, NULL);
    borderPtr->lightGC = RbcGetPrivateGC(tkwin, 0, NULL);
