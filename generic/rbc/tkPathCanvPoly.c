...
static int    CoordsForPolygonline(Tcl_Interp *interp, Tk_PathCanvas canvas,
            int closed, int objc, Tcl_Obj *const objv[],
...
ComputePpolyBbox(Tk_PathCanvas canvas, PpolyItem *ppolyPtr)
{
    ...
    itemPtr->totalBbox = TkPathGetGenericPathTotalBboxFromBare(ppolyPtr->atomPtr,
            &style, &itemPtr->bbox);
    ...
    Drawable drawable, int x, int y, int width, int height)
{
     ...
     */
    TkPathDisplayArrow(canvas, &ppolyPtr->startarrow, &style, &m, &itemPtr->bbox);
