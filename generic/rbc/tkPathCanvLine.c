...
static TkPathAtom * MakePathAtoms(PlineItem *plinePtr);
...
ComputePlineBbox(Tk_PathCanvas canvas, PlineItem *plinePtr)
{
    ...
    itemPtr->bbox = r;
    itemPtr->totalBbox = TkPathGetGenericPathTotalBboxFromBare(NULL, &style, &r);
...
MakePathAtoms(PlineItem *plinePtr)
{
    ...
    atomPtr->nextPtr = TkPathNewLineToAtom(plinePtr->coords.x2, plinePtr->coords.y2);
    return atomPtr;
}
