...
static int    PathStyleCget(Tcl_Interp *interp, Tk_Window tkwin,
            int objc, Tcl_Obj * const objv[],
            Tcl_HashTable *hashTablePtr);
static int    PathStyleConfigure(Tcl_Interp *interp, Tk_Window tkwin,
            int objc, Tcl_Obj * const objv[],
            Tcl_HashTable *hashTablePtr,
            Tcl_HashTable *gradTablePtr);
static int    PathStyleCreate(Tcl_Interp *interp, Tk_Window tkwin,
            int objc, Tcl_Obj * const objv[],
            Tcl_HashTable *styleTablePtr,
            Tcl_HashTable *gradTablePtr, char *tokenName);
static int    PathStyleDelete(Tcl_Interp *interp, Tcl_Obj *obj,
            Tcl_HashTable *hashTablePtr, Tk_Window tkwin);
static int    PathStyleInUse(Tcl_Interp *interp, Tcl_Obj *obj,
            Tcl_HashTable *tablePtr);
static void   PathStyleNames(Tcl_Interp *interp,
            Tcl_HashTable *hashTablePtr);
...
int
TkPathCanvasStyleObjCmd(Tcl_Interp* interp, TkPathCanvas *canvasPtr,
    int objc, Tcl_Obj* const objv[])
{
    int index;
    int result = TCL_OK;
    ...
    switch (index) {
        ...
        return TCL_ERROR;
            ...
            sprintf(str, "%s%d", TK_PATHCMD_STYLE, canvasPtr->styleUid++);
...
void
TkPathDashFree(TkPathDash *dashPtr)
{
    if (dashPtr->array) {
...
static Tk_OptionSpec styleOptionSpecs[] = {
    ...
    TK_OPTION_NULL_OK, (ClientData) &dashCO, TK_PATH_STYLE_OPTION_STROKE_DASHARRAY},
...
void
TkPathStyleInit(Tcl_Interp *interp)
{
    InterpData *dataPtr =
...
void
TkPathStylesFree(Tk_Window tkwin, Tcl_HashTable *hashTablePtr)
{
    Tcl_HashEntry     *hPtr;
    Tcl_HashSearch    search;
    char        *recordPtr;
