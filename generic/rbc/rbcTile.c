...
static void DestroyClient (RbcTileClient *clientPtr);
static void DestroyTile (Tile *tilePtr);
static void RedrawTile (Tk_Window tkwin, Tile *tilePtr);
static Tile *CreateTile (Tcl_Interp *interp, Tk_Window tkwin, const char *imageName);
static RbcTileClient *CreateClient (Tcl_Interp *interp, Tk_Window tkwin, const char *name);
#ifdef _WIN32
static void TileRegion (HDC srcDC, HDC destDC, HDC maskDC, RbcTileClient *clientPtr, int x, int y, int width, int height);
#endif
/*
 *--------------------------------------------------------------
 *
    ...
    Tile *tilePtr)
{
    ...
    photo = Tk_FindPhoto(tilePtr->interp,
                          RbcNameOfImage(tilePtr->tkImage));
    if (photo != NULL) {
...
RbcGetTile(
    ...
    RbcTile *tokenPtr) /* (out) Returns the allocated tile token. */
{
    RbcTileClient *clientPtr;
...
void
RbcFreeTile(
    RbcTileClient *clientPtr) /* Tile to be deleted */
{
