...
static TkPathAtom * MakePathAtoms(PrectItem *prectPtr);
        ...
        int objc, Tcl_Obj *const objv[])
{
    ...
    if (TkPathCoordsForRectangularItems(interp, canvas, &itemPtr->bbox, i, objv) != TCL_OK) {
        goto error;
    }
        ...
        int objc, Tcl_Obj *const objv[])
{
    ...
    result = TkPathCoordsForRectangularItems(interp, canvas, &itemPtr->bbox, objc, objv);
...
ComputePrectBbox(Tk_PathCanvas canvas, PrectItem *prectPtr)
{
    ...
    itemPtr->totalBbox = TkPathGetGenericPathTotalBboxFromBare(NULL, &style, &itemPtr->bbox);
    TkPathSetGenericPathHeaderBbox(&itemExPtr->header, style.matrixPtr, &itemPtr->totalBbox);
