...
TkPathAtom *
TkPathMakePathAtomsFromArrow(TkPathArrowDescr *arrowDescr)
{
...
void
TkPathDisplayArrow(Tk_PathCanvas canvas, TkPathArrowDescr *arrowDescr,
        Tk_PathStyle *const style, TkPathMatrix *mPtr, TkPathRect *bboxPtr)
{
...
void
TkPathPaintArrow(TkPathContext context, TkPathArrowDescr *arrowDescr,
       Tk_PathStyle *const style, TkPathRect *bboxPtr)
{
    if (arrowDescr->arrowEnabled && arrowDescr->arrowPointsPtr != NULL) {
        Tk_PathStyle arrowStyle = *style;
        TkPathColor fc;
...
void
TkPathArrowDescrInit(TkPathArrowDescr *descrPtr)
{
...
void
TkPathIncludeArrowPointsInRect(TkPathRect *bbox, TkPathArrowDescr *arrowDescrPtr)
{
    if (arrowDescrPtr->arrowEnabled && arrowDescrPtr->arrowPointsPtr) {
        ...
        !isnan(arrowDescrPtr->arrowPointsPtr[i].y))
                TkPathIncludePointInRect(bbox, arrowDescrPtr->arrowPointsPtr[i].x,
                   arrowDescrPtr->arrowPointsPtr[i].y);
    }
}

void
TkPathIncludeArrowPoints(Tk_PathItem *itemPtr, TkPathArrowDescr *arrowDescrPtr)
{
...
void
TkPathPreconfigureArrow(TkPathPoint *pf, TkPathArrowDescr *arrowDescr)
{
...
TkPathPoint
TkPathConfigureArrow(TkPathPoint pf, TkPathPoint pl, TkPathArrowDescr *arrowDescr,
...
void
TkPathTranslateArrow(TkPathArrowDescr *arrowDescr, double deltaX, double deltaY)
{
    if (arrowDescr->arrowPointsPtr != NULL) {
        int i;
        for (i = 0; i < PTS_IN_ARROW; i++) {
...
void
TkPathScaleArrow(TkPathArrowDescr *arrowDescr, double originX, double originY,
...
void
TkPathFreeArrow(TkPathArrowDescr *arrowDescr)
{
    if (arrowDescr->arrowPointsPtr != NULL) {
        ckfree((char *)arrowDescr->arrowPointsPtr);
        arrowDescr->arrowPointsPtr = NULL;
...
int
TkPathGetSegmentsFromPathAtomList(TkPathAtom *firstAtom,
