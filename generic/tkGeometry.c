 ...
 * each content window managed by Tk_MaintainGeometry, there is one of these structures
 * associated with its container.
 */

typedef struct MaintainContent {
    Tk_Window content;		/* The content window being positioned. */
    Tk_Window container;		/* The container that determines content window's
				 * position; it must be a descendant of
				 * the content window's parent. */
...
/*
 * For each window that has been specified as a container to Tk_MaintainGeometry,
...
typedef struct MaintainContainer {
    ...
    int checkScheduled;		/* Non-zero means that there is already a call
				 ...
				 * handler. */
    MaintainContent *contentPtr;	/* First in list of all content windows associated with
				 * this container. */
} MaintainContainer;
...
TkSetGeometryContainer(
    ...
    Tk_Window tkwin,		/* Window that will have geometry container
				 * set. */
    const char *container)		/* The container identity. */
{
    register TkWindow *winPtr = (TkWindow *) tkwin;

    if (winPtr->geomMgrName != NULL &&
	    strcmp(winPtr->geomMgrName, container) == 0) {
	return TCL_OK;
    }
    if (winPtr->geomMgrName != NULL) {
	if (interp != NULL) {
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf(
		    ...
		    " has content managed by %s",
		    container, Tk_PathName(tkwin), winPtr->geomMgrName));
	    Tcl_SetErrorCode(interp, "TK", "GEOMETRY", "FIGHT", NULL);
	}
	return TCL_ERROR;
    }

    winPtr->geomMgrName = ckalloc(strlen(container) + 1);
    strcpy(winPtr->geomMgrName, container);
    return TCL_OK;
}

/*
 ...
 *	the content window's parent) is mapped, unmapped, or moved, then the content will
...
void
Tk_MaintainGeometry(
    Tk_Window content,		/* Content window for geometry management. */
    Tk_Window container,		/* Container for content; must be a descendant of
				 * content's parent. */
    int x, int y,		/* Desired position of content within container. */
    int width, int height)	/* Desired dimensions for content. */
{
    Tcl_HashEntry *hPtr;
    MaintainContainer *containerPtr;
    register MaintainContent *contentPtr;
    int isNew, map;
    Tk_Window ancestor, parent;
    TkDisplay *dispPtr = ((TkWindow *) container)->dispPtr;

    ((TkWindow *)content)->maintainerPtr = (TkWindow *)container;

    ((TkWindow *)content)->maintainerPtr = (TkWindow *)container;
    if (container == Tk_Parent(content)) {
	/*
	 * If the content is a direct descendant of the container, don't bother
	...
	Tk_MoveResizeWindow(content, x, y, width, height);

	/*
	 * Map the content if the container is already mapped; otherwise, wait
	 * until the container is mapped later (in which case mapping the content
	 * is taken care of elsewhere).
	 */

	if (Tk_IsMapped(container)) {
	    Tk_MapWindow(content);
	}
	return;
    }
	...
	containerPtr = ckalloc(sizeof(MaintainContainer));
    ...
    /*
     * Create a MaintainContent structure for the content if there isn't already
     * one.
     */

    for (contentPtr = containerPtr->contentPtr; contentPtr != NULL;
	    contentPtr = contentPtr->nextPtr) {
	if (contentPtr->content == content) {
	    goto gotContent;
	}
    }
    contentPtr = ckalloc(sizeof(MaintainContent));
    contentPtr->content = content;
    contentPtr->container = container;
    contentPtr->nextPtr = containerPtr->contentPtr;
    containerPtr->contentPtr = contentPtr;
    Tk_CreateEventHandler(content, StructureNotifyMask, MaintainContentProc,
	    contentPtr);

    /*
     * Make sure that there are event handlers registered for all the windows
     * between container and content's parent (including container but not content's
 ...
 *	the relationship between content and container is no longer maintained.
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	The content is unmapped and state is released, so that content won't track
 *	container any more. If we weren't previously managing content relative to
...
void
Tk_UnmaintainGeometry(
    Tk_Window tkwin,		/* window for geometry management. */
    Tk_Window container)		/* Container for window; must be a descendant of
				 * windows's parent. */
{
    Tcl_HashEntry *hPtr;
    MaintainContainer *containerPtr;
    register MaintainContent *contentPtr, *prevPtr;
    Tk_Window ancestor;
    ...
    ((TkWindow *)tkwin)->maintainerPtr = NULL;
    if (container == Tk_Parent(tkwin)) {
	/*
	 * If the content is a direct descendant of the container,
    ...
    hPtr = Tcl_FindHashEntry(&dispPtr->maintainHashTable, (char *) container);
    if (hPtr == NULL) {
	return;
    }
    containerPtr = Tcl_GetHashValue(hPtr);
    contentPtr = containerPtr->contentPtr;
    if (contentPtr->content == tkwin) {
 ...
 *	maintained content of the container. Or, if the container window is being
...
{
    MaintainContainer *containerPtr = clientData;
	 ...
	 * not to use containerPtr after the last content is deleted, since its
 ...
 *
 *	This procedure is invoked by the Tk event dispatcher in response to
 *	StructureNotify events on a content being managed by
 *	Tk_MaintainGeometry.
 *
 * Results:
 *	None.
 ...
 *	event handlers for this content are deleted.
 *
 *----------------------------------------------------------------------
 */
...
MaintainContentProc(
    ...
    XEvent *eventPtr)		/* Describes what just happened. */
{
    MaintainContent *contentPtr = clientData;
 ...
 *	mapped, or unmapped. Its job is to scan all of the content for the
 ...
 *
 * Side effects:
 *	Content can get repositioned, mapped, or unmapped.
 *
 *----------------------------------------------------------------------
 */
